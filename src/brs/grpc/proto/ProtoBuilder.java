...
public final class ProtoBuilder {
  ...
  public static StatusException buildError(Throwable t) {
    String message =
        t.getMessage() == null ? "Unknown Error: " + t.getClass().toString() : t.getMessage();
    if (t.getMessage() == null) {
      logger.debug("Unknown message for gRPC API exception. Exception:", t);
    }
    return StatusProto.toStatusException(
        Status.newBuilder().setCode(Code.ABORTED_VALUE).setMessage(message).build());
  }
  ...
  public static BrsApi.Account buildAccount(Account account, AccountService accountService) {
    ...
    Account.RewardRecipientAssignment rewardRecipient =
    ...
    return BrsApi.Account.newBuilder()
        ...
        .setRewardRecipient(
            rewardRecipient == null || rewardRecipient.accountId == null
                ? 0
                : rewardRecipient.accountId)
        .addAllAssetBalances(
            accountService.getAssets(account.id, 0, -1).stream()
  ...
  public static BrsApi.Block buildBlock(
      Blockchain blockchain, BlockService blockService, Block block, boolean includeTransactions) {
    BrsApi.Block.Builder builder =
        BrsApi.Block.newBuilder()
            .setId(block.getId())
  ...
  public static BrsApi.BasicTransaction buildBasicTransaction(Transaction transaction) {
    BrsApi.BasicTransaction.Builder builder =
        BrsApi.BasicTransaction.newBuilder()
  ...
  public static BrsApi.Transaction buildUnconfirmedTransaction(Transaction transaction) {
    return BrsApi.Transaction.newBuilder()
        .setTransaction(buildBasicTransaction(transaction))
        .setId(transaction.getId())
        .setTransactionBytes(buildByteString(transaction.getBytes()))
  ...
  public static BrsApi.Alias buildAlias(Alias alias, Alias.Offer offer) {
    BrsApi.Alias.Builder builder =
        BrsApi.Alias.newBuilder()
            .setId(alias.getId())
  ...
  public static BrsApi.EncryptedData buildEncryptedData(EncryptedData encryptedData) {
    if (encryptedData == null)
      return BrsApi.EncryptedData.getDefaultInstance(); // TODO is this needed for all methods?
  ...
  public static EncryptedData parseEncryptedData(BrsApi.EncryptedData encryptedData) {
    return new EncryptedData(
        encryptedData.getData().toByteArray(), encryptedData.getNonce().toByteArray());
  }
  ...
  public static BrsApi.DgsPurchase buildPurchase(
      DigitalGoodsStore.Purchase purchase, DigitalGoodsStore.Goods goods) {
    return BrsApi.DgsPurchase.newBuilder()
        ...
        .addAllFeedback(
            purchase.getFeedbackNotes().stream()
  ...
  public static Transaction parseBasicTransaction(
      Blockchain blockchain, BrsApi.BasicTransaction basicTransaction) throws ApiException {
    try {
      Transaction.Builder transactionBuilder =
          new Transaction.Builder(
                  ((byte) basicTransaction.getVersion()),
                  basicTransaction.getSenderPublicKey().toByteArray(),
                  basicTransaction.getAmount(),
                  basicTransaction.getFee(),
                  basicTransaction.getTimestamp(),
                  ((short) basicTransaction.getDeadline()),
                  Attachment.AbstractAttachment.parseProtobufMessage(
      ...
      if (basicTransaction.getReferencedTransactionFullHash().size() > 0) {
        transactionBuilder.referencedTransactionFullHash(
            basicTransaction.getReferencedTransactionFullHash().toByteArray());
      }
      ...
      for (Any appendix : basicTransaction.getAppendagesList()) {
        try {
          if (appendix.is(BrsApi.MessageAppendix.class)) {
            transactionBuilder.message(
                new Appendix.Message(
                    appendix.unpack(BrsApi.MessageAppendix.class), blockchainHeight));
          } else if (appendix.is(BrsApi.EncryptedMessageAppendix.class)) {
            BrsApi.EncryptedMessageAppendix encryptedMessageAppendix =
                appendix.unpack(BrsApi.EncryptedMessageAppendix.class);
            switch (encryptedMessageAppendix.getType()) {
              case TO_RECIPIENT:
                transactionBuilder.encryptedMessage(
                    new Appendix.EncryptedMessage(encryptedMessageAppendix, blockchainHeight));
                break;
              case TO_SELF:
                transactionBuilder.encryptToSelfMessage(
                    new Appendix.EncryptToSelfMessage(encryptedMessageAppendix, blockchainHeight));
                break;
              default:
                throw new ApiException(
                    "Invalid encrypted message type: " + encryptedMessageAppendix.getType().name());
            }
          } else if (appendix.is(BrsApi.PublicKeyAnnouncementAppendix.class)) {
            transactionBuilder.publicKeyAnnouncement(
                new Appendix.PublicKeyAnnouncement(
                    appendix.unpack(BrsApi.PublicKeyAnnouncementAppendix.class), blockchainHeight));
          }
