...
public class CompleteBasicTransactionHandler
    implements GrpcApiHandler<BrsApi.BasicTransaction, BrsApi.BasicTransaction> {

  private final TimeService timeService;
  private final TransactionProcessor transactionProcessor;
  private final Blockchain blockchain;

  public CompleteBasicTransactionHandler(
      TimeService timeService, TransactionProcessor transactionProcessor, Blockchain blockchain) {
  ...
  @Override
  public BrsApi.BasicTransaction handleRequest(BrsApi.BasicTransaction basicTransaction)
      throws Exception {
    try {
      BrsApi.BasicTransaction.Builder builder = basicTransaction.toBuilder();
      Attachment.AbstractAttachment attachment =
          Attachment.AbstractAttachment.parseProtobufMessage(basicTransaction.getAttachment());
      ...
      if (builder.getSenderId() == 0) {
        builder.setSenderId(
            BurstCrypto.getInstance()
                .getBurstAddressFromPublic(builder.getSenderPublicKey().toByteArray())
                .getBurstID()
                .getSignedLongId());
      }
