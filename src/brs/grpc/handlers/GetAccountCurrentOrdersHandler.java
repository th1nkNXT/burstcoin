...
public class GetAccountCurrentOrdersHandler
    implements GrpcApiHandler<BrsApi.GetAccountOrdersRequest, BrsApi.Orders> {
  ...
  public BrsApi.Orders handleRequest(BrsApi.GetAccountOrdersRequest request) throws Exception {
    ...
    switch (request.getOrderType()) {
      case ASK:
        (assetId == 0
                ? assetExchange.getAskOrdersByAccount(accountId, firstIndex, lastIndex)
                : assetExchange.getAskOrdersByAccountAsset(
                    accountId, assetId, firstIndex, lastIndex))
      ...
      case BID:
        (assetId == 0
                ...
                : assetExchange.getBidOrdersByAccountAsset(
                    accountId, assetId, firstIndex, lastIndex))
            .forEach(order -> builder.addOrders(ProtoBuilder.buildOrder(order)));
        break;
      default:
