...
public class GetAccountsHandler
    implements GrpcApiHandler<BrsApi.GetAccountsRequest, BrsApi.Accounts> {
  ...
  public BrsApi.Accounts handleRequest(BrsApi.GetAccountsRequest request) throws Exception {
    ...
    if (!Objects.equals(request.getName(), "")) {
      ...
      if (request.getIncludeAccounts()) {
        accounts.forEach(
            account -> builder.addAccounts(ProtoBuilder.buildAccount(account, accountService)));
      }
    }
    if (request.getRewardRecipient() != 0) {
      Collection<Account.RewardRecipientAssignment> accounts =
          accountService.getAccountsWithRewardRecipient(request.getRewardRecipient());
            ...
            assignment ->
                builder.addAccounts(
                    ProtoBuilder.buildAccount(
                        accountService.getAccount(assignment.getAccountId()), accountService)));
      }
    }
