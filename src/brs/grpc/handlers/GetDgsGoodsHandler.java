...
public class GetDgsGoodsHandler
    implements GrpcApiHandler<BrsApi.GetDgsGoodsRequest, BrsApi.DgsGoods> {
  ...
  public BrsApi.DgsGoods handleRequest(BrsApi.GetDgsGoodsRequest request) throws Exception {
    ...
    if (sellerId == 0) {
      ...
      digitalGoodsStoreService
          .getSellerGoods(sellerId, inStockOnly, firstIndex, lastIndex)
          .forEach(goods -> builder.addGoods(ProtoBuilder.buildGoods(goods)));
    }
