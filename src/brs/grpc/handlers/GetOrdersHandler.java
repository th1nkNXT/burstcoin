  ...
  public BrsApi.Orders handleRequest(BrsApi.GetOrdersRequest request) throws Exception {
    ...
    if (assetId == 0) {
      ...
      switch (request.getOrderType()) {
        case ASK:
          assetExchange
              .getAllAskOrders(firstIndex, lastIndex)
              .forEach(order -> builder.addOrders(ProtoBuilder.buildOrder(order)));
          break;
        case BID:
          assetExchange
              .getAllBidOrders(firstIndex, lastIndex)
              .forEach(order -> builder.addOrders(ProtoBuilder.buildOrder(order)));
          break;
        default:
      ...
      switch (request.getOrderType()) {
        case ASK:
          assetExchange
              .getSortedAskOrders(assetId, firstIndex, lastIndex)
              .forEach(order -> builder.addOrders(ProtoBuilder.buildOrder(order)));
          break;
        case BID:
          assetExchange
              .getSortedBidOrders(assetId, firstIndex, lastIndex)
              .forEach(order -> builder.addOrders(ProtoBuilder.buildOrder(order)));
          break;
        default:
          throw new ApiException("Order type unset");
