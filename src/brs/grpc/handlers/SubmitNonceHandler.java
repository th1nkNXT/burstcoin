...
public class SubmitNonceHandler
    implements GrpcApiHandler<BrsApi.SubmitNonceRequest, BrsApi.SubmitNonceResponse> {
  ...
  @Override
  public BrsApi.SubmitNonceResponse handleRequest(BrsApi.SubmitNonceRequest request)
      throws Exception {
    ...
    if (Objects.equals(secret, "")) {
      if (passphrases.containsKey(accountId)) {
        ...
        throw new ApiException(
            "Missing Passphrase and account passphrase not in solo mining config");
      }
    }
    ...
    return BrsApi.SubmitNonceResponse.newBuilder()
        .setDeadline(generatorState.getDeadline().longValueExact())
        .build();
  }

  public static void verifySecretAccount(
      AccountService accountService, Blockchain blockchain, Account secretAccount, long accountId)
      throws ApiException {
    Account genAccount;
    if (accountId != 0) {
    ...
    if (genAccount != null) {
      Account.RewardRecipientAssignment assignment =
          accountService.getRewardRecipientAssignment(genAccount);
      long rewardId;
      if (assignment == null) {
