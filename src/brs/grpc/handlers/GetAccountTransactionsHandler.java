...
public class GetAccountTransactionsHandler
    implements GrpcApiHandler<BrsApi.GetAccountTransactionsRequest, BrsApi.Transactions> {
  ...
  @Override
  public BrsApi.Transactions handleRequest(BrsApi.GetAccountTransactionsRequest request)
      throws Exception {
    ...
    blockchain
        .getTransactions(
            account, numberOfConfirmations, type, subtype, timestamp, firstIndex, lastIndex, true)
        .forEach(
            transaction ->
                builder.addTransactions(ProtoBuilder.buildTransaction(transaction, currentHeight)));
