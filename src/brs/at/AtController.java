...
public abstract class AtController {
  ...
  private static final Logger debugLogger =
      Burst.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG)
          ? logger
  ...
  private static int runSteps(AtMachineState state) {
    ...
    AtMachineProcessor processor =
        new AtMachineProcessor(
            state, Burst.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG));
    ...
    while (state.getMachineState().steps
            + (numSteps =
                getNumSteps(
                    state.getApCode().get(state.getMachineState().pc),
                    state.getCreationBlockHeight()))
      ...
      if (rc >= 0) {
        ...
        if (rc == -1) debugLogger.debug("error: overflow");
        else if (rc == -2) debugLogger.debug("error: invalid code");
        else debugLogger.debug("unexpected error");
  ...
  private static int getNumSteps(byte op, int height) {
    if (op >= 0x32 && op < 0x38) return (int) AtConstants.getInstance().apiStepMultiplier(height);
  ...
  private static void listCode(AtMachineState state, boolean disassembly, boolean determineJumps) {
    ...
    AtMachineProcessor machineProcessor =
        new AtMachineProcessor(
            state, Burst.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG));
  ...
  public static int checkCreationBytes(byte[] creation, int height) throws AtException {
    if (creation == null) throw new AtException("Creation bytes cannot be null");
  ...
  public static AtBlock getCurrentBlockATs(int freePayload, int blockHeight, long generatorId) {
    ...
    while (payload <= freePayload - costOfOneAT && keys.hasNext()) {
      ...
      if (at.freezeOnSameBalance()
          && (atAccountBalance - atStateBalance < at.minActivationAmount())) {
        continue;
      }

      if (atAccountBalance
          >= AtConstants.getInstance().stepFee(at.getCreationBlockHeight())
  ...
  public static AtBlock validateATs(byte[] blockATs, int blockHeight, long generatorId)
      throws AtException {
    if (blockATs == null) {
    ...
    for (Map.Entry<ByteBuffer, byte[]> entry : ats.entrySet()) {
      ...
      try {
        ...
        if (at.freezeOnSameBalance()
            && (atAccountBalance - at.getgBalance() < at.minActivationAmount())) {
  ...
  private static LinkedHashMap<ByteBuffer, byte[]> getATsFromBlock(byte[] blockATs)
      throws AtException {
    if (blockATs.length > 0 && blockATs.length % (getCostOfOneAT()) != 0) {
      throw new AtException(
          "blockATs must be a multiple of cost of one AT ( " + getCostOfOneAT() + " )");
    }
  ...
  // platform based
  private static long makeTransactions(AT at, int blockHeight, long generatorId)
      throws AtException {
    long totalAmount = 0;
