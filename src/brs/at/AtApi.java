...
interface AtApi {
  ...
  /**
   * @param state AT machine state
   * @return bool if A is a valid tx with @addr to tx type 0: normal tx 1: message tx
   */
  ...
  /**
   * @param state AT machine state
   * @return bool if A is a valid tx with @addr to the tx random id random id is a 64bit signed
   *     value (always positive) and this is a blocking function
   */
  long getRandomIdForTxInA(AtMachineState state);

  /**
   * bool if A is a valid tx with B to the tx message if a tx is not a message tx then this will
   * zero out the B value
   *
   * @param state AT machine state
   */
  ...
  /**
   * sets @addr to the balance it had last had when running this amount does not include any
   * additional amounts sent to the AT between "execution events"
   *
   * @param state AT machine state
   */
  ...
  /**
   * bool if B is a valid address then send it $addr amount if this amount is greater than the AT's
   * balance then it will also return false
   *
   * @param state AT machine state
   */
