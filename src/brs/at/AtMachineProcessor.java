...
class AtMachineProcessor {
  ...
  public AtMachineProcessor(AtMachineState machineData, boolean enableLogger) {
    ...
    this.logger =
        enableLogger ? LoggerFactory.getLogger(AtMachineProcessor.class) : NOPLogger.NOP_LOGGER;
  }

  private int getFun() {

    if (machineData.getMachineState().pc + 2 >= machineData.getcSize()) return -1;
    else {
  ...
  private int getAddr(boolean isCode) {
    ...
    fun.addr1 =
        (machineData.getApCode())
            .getInt((machineData.getApCode()).position() + machineData.getMachineState().pc + 1);
  ...
  private int getFunAddrs() {
    ...
    if (!validAddr(fun.addr3, false) || !validAddr(fun.addr2, false)) {
      return -1;
    }
  ...
  private boolean validAddr(int addr, boolean isCode) {
    ...
    if (!isCode
        && (((long) addr) * 8 + 8 > ((long) Integer.MAX_VALUE)
  ...
  int processOp(boolean disassemble, boolean determineJumps) {
    ...
    if (op > 0 && disassemble && !determineJumps && logger.isDebugEnabled()) {
      ...
      if (machineData.getMachineState().pc == machineData.getMachineState().opc) logger.debug("* ");
      else logger.debug("  ");
    }

    if (op == OpCode.E_OP_CODE_NOP) {
      if (disassemble) {
        if (!determineJumps && logger.isDebugEnabled()) logger.debug("NOP");
        ++rc;
      } else {
        ++rc;
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          ...
          machineData
              .getApData()
              .putLong(fun.addr1 * 8, machineData.getApData().getLong(fun.addr2 * 8));
          machineData.getApData().clear();
        }
      }
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          ...
          if (op == OpCode.E_OP_CODE_DIV_DAT && val == 0) rc = -2;
          else {
            machineData.getMachineState().pc += rc;
            if (op == OpCode.E_OP_CODE_ADD_DAT) {
      ...
      if (rc == 0) {
        ...
        if (disassemble) {
          ...
          if (!validAddr((int) addr, false)) rc = -1;
          else {
            machineData.getMachineState().pc += rc;
      ...
      if (Burst.getFluxCapacitor()
          .getValue(FluxValues.SIGNUM, machineData.getCreationBlockHeight())) {
        rc = get3Addrs();
        if (rc == 0 || disassemble) {
          rc = 13;
          if (disassemble) {
            ...
            machineData
                .getApData()
                .putLong(fun.addr1 * 8, machineData.getApData().getLong(addr * 8));
            machineData.getMachineState().pc += rc;
            machineData.getApData().clear();
          }
        }
        ...
        if (rc == 0 || disassemble) {
          ...
          if (rc == 0 || disassemble) {
            ...
            if (!validAddr((int) addr, false)) {
              ...
              machineData
                  .getApData()
                  .putLong(addr1 * 8, machineData.getApData().getLong((int) addr * 8));
              machineData.getApData().clear();
            }
          }
        }
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          if (!determineJumps) {
            if (op == OpCode.E_OP_CODE_PSH_DAT) logger.debug("PSH $");
            else logger.debug("POP @");
            if (logger.isDebugEnabled()) {
              logger.debug(String.format("%8s", fun.addr1).replace(' ', '0'));
            }
          }
        } else if ((op == OpCode.E_OP_CODE_PSH_DAT
                && machineData.getMachineState().us == (machineData.getcUserStackBytes() / 8))
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          ...
          if (machineData.getMachineState().cs == (machineData.getcCallStackBytes() / 8)) rc = -1;
      ...
      if (disassemble) {
        if (!determineJumps) logger.debug("RET\n");
      } else {
        if (machineData.getMachineState().cs == 0) rc = -1;
        else {
          long val =
              machineData
                  .getApData()
      ...
      if (rc == 0) {
        ...
        if (disassemble) {
          ...
          if (!validAddr((int) addr, false)) rc = -1;
            ...
            machineData
                .getApData()
                .putLong((int) addr * 8, machineData.getApData().getLong(fun.addr2 * 8));
            machineData.getApData().clear();
          }
        }
      }
      ...
      if (Burst.getFluxCapacitor()
        ...
        if (rc == 0 || disassemble) {
          ...
          if (disassemble) {
            ...
            int addr =
                (int)
                    (machineData.getApData().getLong(fun.addr1 * 8)
                        + machineData.getApData().getLong(fun.addr2 * 8));
            machineData
                .getApData()
                .putLong(addr * 8, machineData.getApData().getLong(fun.addr3 * 8));
            machineData.getMachineState().pc += rc;
            machineData.getApData().clear();
          }
        }
        ...
        if (rc == 0 || disassemble) {
          ...
          if (rc == 0 || disassemble) {
            ...
            if (disassemble) {
              ...
              long addr =
                  machineData.getApData().getLong(addr1 * 8)
              ...
              if (!validAddr((int) addr, false)) rc = -1;
                ...
                machineData
                    .getApData()
                    .putLong((int) addr * 8, machineData.getApData().getLong(fun.addr1 * 8));
                machineData.getApData().clear();
              }
            }
          }
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          ...
          if (shift < 0) shift = 0;
          else if (shift > 63) shift = 63;

          if (op == OpCode.E_OP_CODE_SHL_DAT)
            machineData.getApData().putLong(fun.addr1 * 8, val << shift);
          else machineData.getApData().putLong(fun.addr1 * 8, val >>> shift);
        }
      }
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          if (!determineJumps) {
            if (op == OpCode.E_OP_CODE_BZR_DAT) logger.debug("BZR $");
            else logger.debug("BNZ $");

            if (logger.isDebugEnabled()) {
              logger.debug(
                  "{}, :{}",
                  String.format("%8x", fun.addr1).replace(' ', '0'),
                  String.format("%8x", machineData.getMachineState().pc + fun.off)
                      .replace(' ', '0'));
            }
          }
        } else {
          ...
          if ((op == OpCode.E_OP_CODE_BZR_DAT && val == 0)
            ...
            if (machineData
                .getMachineState()
                .jumps
                .contains(machineData.getMachineState().pc + fun.off))
              machineData.getMachineState().pc += fun.off;
            else rc = -2;
          } else machineData.getMachineState().pc += rc;
        }
      }
    } else if (op == OpCode.E_OP_CODE_BGT_DAT
        || op == OpCode.E_OP_CODE_BLT_DAT
        || op == OpCode.E_OP_CODE_BGE_DAT
        || op == OpCode.E_OP_CODE_BLE_DAT
        || op == OpCode.E_OP_CODE_BEQ_DAT
        || op == OpCode.E_OP_CODE_BNE_DAT) {
      rc = getAddrsOff();
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          if (!determineJumps) {
            if (op == OpCode.E_OP_CODE_BGT_DAT) logger.debug("BGT $");
            else if (op == OpCode.E_OP_CODE_BLT_DAT) logger.debug("BLT $");
            else if (op == OpCode.E_OP_CODE_BGE_DAT) logger.debug("BGE $");
            else if (op == OpCode.E_OP_CODE_BLE_DAT) logger.debug("BLE $");
            else if (op == OpCode.E_OP_CODE_BEQ_DAT) logger.debug("BEQ $");
            else logger.debug("BNE $");
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          ...
          int maxNumBlocks =
              (int)
                  AtConstants.getInstance()
                      .getMaxWaitForNumOfBlocks(machineData.getCreationBlockHeight());
          if (numBlocks > maxNumBlocks) numBlocks = maxNumBlocks;
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          if (!determineJumps) {
            if (op == OpCode.E_OP_CODE_FIZ_DAT) logger.debug("FIZ @");
            else logger.debug("STZ @");
      ...
      if (disassemble) {
        if (!determineJumps) {
          if (op == OpCode.E_OP_CODE_FIN_IMD) logger.debug("FIN\n");
          else logger.debug("STP");
        }
      ...
      if (disassemble) {
        if (!determineJumps && logger.isDebugEnabled()) logger.debug("PCS");
      } else {
        machineData.getMachineState().pc += rc;
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          if (!determineJumps && logger.isDebugEnabled()) logger.debug("FUN {}", fun.fun);
        } else {
          machineData.getMachineState().pc += rc;
      ...
      if (rc == 0 || disassemble) {
        ...
        if (disassemble) {
          ...
          machineData
              .getApData()
              .putLong(fun.addr1 * 8, AtApiController.func(fun.fun, machineData));
          machineData.getApData().clear();
        }
      }
      ...
      if (rc == 0) {
        ...
        if (disassemble) {
          ...
          if (op != OpCode.E_OP_CODE_EXT_FUN_RET_DAT_2)
            machineData
                .getApData()
                .putLong((fun.addr3 * 8), AtApiController.func1(fun.fun, val, machineData));
          else {
            long val2 = machineData.getApData().getLong((fun.addr1 * 8));
            machineData
                .getApData()
                .putLong((fun.addr3 * 8), AtApiController.func2(fun.fun, val, val2, machineData));
          }
          machineData.getApData().clear();
        }
      }
    ...
    if (rc == -1 && disassemble && !determineJumps) logger.debug("\n(overflow)");

    if (rc == -2 && disassemble && !determineJumps) logger.debug("\n(invalid op)");
