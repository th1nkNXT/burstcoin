...
public class AtApiPlatformImpl extends AtApiImpl {
  ...
  private static Long findTransaction(
      int startHeight, int endHeight, Long atID, int numOfTx, long minAmount) {
    return Burst.getStores()
        .getAtStore()
        .findTransaction(startHeight, endHeight, atID, numOfTx, minAmount);
  }

  private static int findTransactionHeight(
      Long transactionId, int height, Long atID, long minAmount) {
    return Burst.getStores()
        .getAtStore()
        .findTransactionHeight(transactionId, height, atID, minAmount);
  }
  ...
  public long getAmountForTxInA(AtMachineState state) {
    ...
    if ((tx.getMessage() == null
            || Burst.getFluxCapacitor().getValue(FluxValues.AT_FIX_BLOCK_2, state.getHeight()))
  ...
  public long getTimestampForTxInA(AtMachineState state) {
    ...
    int txHeight =
        findTransactionHeight(
            txId, blockHeight, AtApiHelper.getLong(b), state.minActivationAmount());
  ...
  public long getRandomIdForTxInA(AtMachineState state) {
    ...
    if (blockHeight - txBlockHeight
        < AtConstants.getInstance()
            .blocksForRandom(blockHeight)) { // for tests - for real case 1440
      state.setWaitForNumberOfBlocks(
          (int) AtConstants.getInstance().blocksForRandom(blockHeight)
  ...
  public void sendToAddressInB(long val, AtMachineState state) {
    ...
    if (val < state.getgBalance()) {
      ...
      AtTransaction tx =
          new AtTransaction(state.getId(), state.getB1().clone(), state.getgBalance(), null);
      state.addTransaction(tx);
  ...
  public long addMinutesToTimestamp(long val1, long val2, AtMachineState state) {
    ...
    int addHeight =
        height + (int) (val2 / AtConstants.getInstance().averageBlockMinutes(state.getHeight()));
