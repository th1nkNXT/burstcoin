...
public class BurstGUI extends JFrame {
          ...
          public void windowClosing(WindowEvent e) {
            if (trayIcon == null) {
              if (JOptionPane.showConfirmDialog(
                      BurstGUI.this,
                      "This will stop the Signum node. Are you sure?",
                      "Exit and stop Signum",
              ...
              trayIcon.displayMessage(
                  "Signum GUI closed", "Note that Signum is still running", MessageType.INFO);
              setVisible(false);
            }
          }
        });
                  ...
                  if (block != null) {
                    ...
                    infoLable.setText(
                        "Latest block: "
                            + block.getHeight()
                            + " Timestamp: "
                            + DATE_FORMAT.format(blockDate));

                    Date now = new Date();
                    int missingBlocks =
                        (int)
                            ((now.getTime() - blockDate.getTime())
                                / (Constants.BURST_BLOCK_TIME * 1000));
  ...
  private TrayIcon createTrayIcon() {
    ...
    JButton openPhoenixButton =
        new JButton(
            openPheonixWalletItem.getLabel(),
            IconFontSwing.buildIcon(FontAwesome.FIRE, 18, iconColor));
    JButton openClassicButton =
        new JButton(
            openClassicWalletItem.getLabel(),
            IconFontSwing.buildIcon(FontAwesome.WINDOW_RESTORE, 18, iconColor));
    JButton editConfButton =
        new JButton("Edit conf file", IconFontSwing.buildIcon(FontAwesome.PENCIL, 18, iconColor));
    JButton popOff10Button =
        new JButton(
            "Pop off 10 blocks", IconFontSwing.buildIcon(FontAwesome.STEP_BACKWARD, 18, iconColor));
    JButton popOff100Button =
        new JButton(
            "Pop off 100 blocks", IconFontSwing.buildIcon(FontAwesome.BACKWARD, 18, iconColor));
    // TODO: find a way to actually store permanently the max block available to pop-off, otherwise
    // we can break it
    ...
    try {
      TrayIcon newTrayIcon =
          new TrayIcon(
              Toolkit.getDefaultToolkit().createImage(BurstGUI.class.getResource(ICON_LOCATION)),
              "Burst Reference Software",
              popupMenu);
      newTrayIcon.setImage(
          newTrayIcon
              .getImage()
              .getScaledInstance(newTrayIcon.getSize().width, -1, Image.SCALE_SMOOTH));
      newTrayIcon.addActionListener(e -> openWebUi(false));

      SystemTray systemTray = SystemTray.getSystemTray();
      systemTray.add(newTrayIcon);

      newTrayIcon.displayMessage(
          "Signum Running",
          "Signum is running on background, use this icon to interact with it.",
          MessageType.INFO);
  ...
  private void popOff(int blocks) {
    ...
    int height =
        blocks > 0
            ? Burst.getBlockchain().getLastBlock().getHeight() - blocks
            : Burst.getBlockchainProcessor().getMinRollbackHeight();
  ...
  private void editConf() {
    ...
    if (!file.exists()) {
      JOptionPane.showMessageDialog(
          this,
          "Could not find conf file: " + Burst.DEFAULT_PROPERTIES_NAME,
          "File not found",
  ...
  private void runBrs() {
    try {
      ...
      try {
        ...
        if (Burst.getBlockchain() == null) onBrsStopped();
      } catch (Exception t) {
  ...
  private void onTestNetEnabled() {
    ...
    if (trayIcon != null) trayIcon.setToolTip(trayIcon.getToolTip() + " (TESTNET)");
  }

  private void onBrsStopped() {
    SwingUtilities.invokeLater(() -> setTitle(getTitle() + " (STOPPED)"));
    if (trayIcon != null) trayIcon.setToolTip(trayIcon.getToolTip() + " (STOPPED)");
  }
  ...
  private static class TextAreaOutputStream extends OutputStream {
    ...
    private void writeString(String string) {
      ...
      if (line.contains("\n")) {
        ...
        if (textArea != null) SwingUtilities.invokeLater(() -> textArea.append(line));
