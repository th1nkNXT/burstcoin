...
public class SqlAssetTransferStore implements AssetTransferStore {
  ...
  private static final BurstKey.LongKeyFactory<AssetTransfer> transferDbKeyFactory =
      new DbKey.LongKeyFactory<AssetTransfer>(ASSET_TRANSFER.ID) {
  ...
  public SqlAssetTransferStore(DerivedTableManager derivedTableManager) {
    assetTransferTable =
        new EntitySqlTable<AssetTransfer>(
            "asset_transfer",
        ...
        ctx -> {
          ctx.insertInto(
              ...
              .values(
                  ...
                  assetTransfer.getSenderId(),
                  assetTransfer.getRecipientId(),
                  assetTransfer.getQuantityQNT(),
                  assetTransfer.getTimestamp(),
                  assetTransfer.getHeight())
              .execute();
        });
  }
    ...
    return Db.useDSLContext(
        ctx -> {
          SelectQuery selectQuery =
              ctx.selectFrom(ASSET_TRANSFER)
                  .where(ASSET_TRANSFER.SENDER_ID.eq(accountId))
                  .unionAll(
                      ctx.selectFrom(ASSET_TRANSFER)
                          .where(
                              ASSET_TRANSFER
  ...
  public Collection<AssetTransfer> getAccountAssetTransfers(
    ...
    return Db.useDSLContext(
        ctx -> {
          SelectQuery<AssetTransferRecord> selectQuery =
              ctx.selectFrom(ASSET_TRANSFER)
                  .where(
                      ASSET_TRANSFER
                          .SENDER_ID
                  ...
                  .unionAll(
                      ctx.selectFrom(ASSET_TRANSFER)
                          .where(ASSET_TRANSFER.RECIPIENT_ID.eq(accountId))
                          .and(ASSET_TRANSFER.SENDER_ID.ne(accountId))
                          .and(ASSET_TRANSFER.ASSET_ID.eq(assetId)))
                  .orderBy(ASSET_TRANSFER.HEIGHT.desc())
                  .getQuery();
    ...
    return Db.useDSLContext(
        ctx -> {
          return ctx.fetchCount(
              ctx.selectFrom(ASSET_TRANSFER).where(ASSET_TRANSFER.ASSET_ID.eq(assetId)));
        });
  }
