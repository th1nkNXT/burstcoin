...
public class SqlAssetStore implements AssetStore {
  ...
  private final BurstKey.LongKeyFactory<Asset> assetDbKeyFactory =
      new DbKey.LongKeyFactory<Asset>(ASSET.ID) {
  ...
  public SqlAssetStore(DerivedTableManager derivedTableManager) {
    assetTable =
        new EntitySqlTable<Asset>(
            "asset", brs.schema.Tables.ASSET, assetDbKeyFactory, derivedTableManager) {
  ...
  private void saveAsset(DSLContext ctx, Asset asset) {
    ctx.insertInto(ASSET)
        ...
        .set(ASSET.HEIGHT, Burst.getBlockchain().getHeight())
        .execute();
  }
