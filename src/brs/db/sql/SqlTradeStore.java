...
public class SqlTradeStore implements TradeStore {
  private final DbKey.LinkKeyFactory<Trade> tradeDbKeyFactory =
      new DbKey.LinkKeyFactory<Trade>("ask_order_id", "bid_order_id") {
  ...
  public SqlTradeStore(DerivedTableManager derivedTableManager) {
    tradeTable =
        new EntitySqlTable<Trade>("trade", TRADE, tradeDbKeyFactory, derivedTableManager) {
  ...
  public Collection<Trade> getAccountTrades(long accountId, int from, int to) {
    return Db.useDSLContext(
        ctx -> {
          SelectQuery<TradeRecord> selectQuery =
              ctx.selectFrom(TRADE)
                  .where(TRADE.SELLER_ID.eq(accountId))
                  .unionAll(
                      ctx.selectFrom(TRADE)
                          .where(TRADE.BUYER_ID.eq(accountId).and(TRADE.SELLER_ID.ne(accountId))))
  ...
  public Collection<Trade> getAccountAssetTrades(long accountId, long assetId, int from, int to) {
    return Db.useDSLContext(
        ctx -> {
          SelectQuery<TradeRecord> selectQuery =
              ctx.selectFrom(TRADE)
                  .where(TRADE.SELLER_ID.eq(accountId).and(TRADE.ASSET_ID.eq(assetId)))
                  .unionAll(
                      ctx.selectFrom(TRADE)
                          .where(TRADE.BUYER_ID.eq(accountId))
                          .and(TRADE.SELLER_ID.ne(accountId))
  ...
  private void saveTrade(DSLContext ctx, Trade trade) {
    ctx.insertInto(
        ...
        .values(
            ...
            trade.getBidOrderHeight(),
            trade.getSellerId(),
            trade.getBuyerId(),
            trade.getQuantityQNT(),
            trade.getPriceNQT(),
            trade.getTimestamp(),
            trade.getHeight())
        .execute();
