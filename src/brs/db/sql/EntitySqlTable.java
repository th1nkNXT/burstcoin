...
public abstract class EntitySqlTable<T> extends DerivedSqlTable implements EntityTable<T> {
  ...
  EntitySqlTable(
      ...
      BurstKey.Factory<T> dbKeyFactory,
          ...
          if (multiversion) {
            ...
            innerQueryC.addConditions(
                innerTableC
                    ...
                    .le(height)
                    .and(innerTableC.field("height", Integer.class).gt(heightField)));
            dbKeyFactory.applySelfJoin(innerQueryC, innerTableC, tableClass);

            query.addConditions(
                latestField
                    .isTrue()
                    .or(DSL.field(DSL.exists(innerQueryB).and(DSL.notExists(innerQueryC)))));
          }
          query.addOrderBy(sort);
  ...
  public void insert(T t) {
    ...
    if (cachedT == null) {
      ...
      throw new IllegalStateException(
          "Different instance found in Db cache, perhaps trying to save an object "
    ...
    Db.useDSLContext(
        ctx -> {
          if (multiversion) {
            ...
            query.addValue(latestField, false);
            query.addConditions(dbKey.getPKConditions(tableClass));
            query.addConditions(latestField.isTrue());
            query.execute();
          }
