...
public class SqlEscrowStore implements EscrowStore {
  private final BurstKey.LongKeyFactory<Escrow> escrowDbKeyFactory =
      new DbKey.LongKeyFactory<Escrow>(ESCROW.ID) {
        @Override
  ...
  public SqlEscrowStore(DerivedTableManager derivedTableManager) {
    escrowTable =
        new VersionedEntitySqlTable<Escrow>(
            "escrow", brs.schema.Tables.ESCROW, escrowDbKeyFactory, derivedTableManager) {
          @Override
    ...
    decisionTable =
        new VersionedEntitySqlTable<Escrow.Decision>(
            "escrow_decision",
            brs.schema.Tables.ESCROW_DECISION,
            decisionDbKeyFactory,
            derivedTableManager) {
          @Override
  ...
  public Collection<Escrow> getEscrowTransactionsByParticipant(Long accountId) {
    ...
    for (Escrow.Decision decision :
        decisionTable.getManyBy(ESCROW_DECISION.ACCOUNT_ID.eq(accountId), 0, -1)) {
    ...
    private SqlDecision(Record record) {
      super(
          decisionDbKeyFactory.newKey(
              record.get(ESCROW_DECISION.ESCROW_ID), record.get(ESCROW_DECISION.ACCOUNT_ID)),
