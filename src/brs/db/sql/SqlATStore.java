...
public class SqlATStore implements ATStore {
  ...
  private final BurstKey.LongKeyFactory<brs.at.AT> atDbKeyFactory =
      new DbKey.LongKeyFactory<brs.at.AT>(AT.ID) {
        @Override
  ...
  private final BurstKey.LongKeyFactory<brs.at.AT.ATState> atStateDbKeyFactory =
      new DbKey.LongKeyFactory<brs.at.AT.ATState>(AT_STATE.AT_ID) {
        @Override
  ...
  public SqlATStore(DerivedTableManager derivedTableManager) {
    atTable =
        new VersionedEntitySqlTable<brs.at.AT>(
            "at", brs.schema.Tables.AT, atDbKeyFactory, derivedTableManager) {
          @Override
    ...
    atStateTable =
        new VersionedEntitySqlTable<brs.at.AT.ATState>(
            "at_state", brs.schema.Tables.AT_STATE, atStateDbKeyFactory, derivedTableManager) {
          @Override
  ...
  private void saveAT(DSLContext ctx, brs.at.AT at) {
    ctx.insertInto(
            ...
            AT.CSIZE,
            AT.DSIZE,
            AT.C_USER_STACK_BYTES,
            AT.C_CALL_STACK_BYTES,
            AT.CREATION_HEIGHT,
            AT.AP_CODE,
            AT.HEIGHT)
        .values(
            AtApiHelper.getLong(at.getId()),
            AtApiHelper.getLong(at.getCreator()),
            at.getName(),
            at.getDescription(),
            at.getVersion(),
            at.getcSize(),
            ...
            at.getcCallStackBytes(),
            at.getCreationBlockHeight(),
            brs.at.AT.compressState(at.getApCodeBytes()),
            Burst.getBlockchain().getHeight())
        .execute();
  }
    ...
    return Db.useDSLContext(
        ctx -> {
          return ctx.fetchExists(
              ctx.selectOne().from(AT).where(AT.ID.eq(id)).and(AT.LATEST.isTrue()));
        });
  }
        ...
        ctx -> {
          return ctx.selectFrom(
                  AT.join(AT_STATE)
                      ...
                      .join(ACCOUNT)
                      .on(AT.ID.eq(ACCOUNT.ID)))
              .where(AT.LATEST.isTrue())
              .and(AT_STATE.LATEST.isTrue())
              .and(ACCOUNT.LATEST.isTrue())
              .and(AT_STATE.NEXT_HEIGHT.lessOrEqual(Burst.getBlockchain().getHeight() + 1))
              .and(
                  ACCOUNT.BALANCE.greaterOrEqual(
                      AtConstants.getInstance().stepFee(Burst.getBlockchain().getHeight())
                          * AtConstants.getInstance()
                              .apiStepMultiplier(Burst.getBlockchain().getHeight())))
              .and(
                  AT_STATE
                      .FREEZE_WHEN_SAME_BALANCE
                      .isFalse()
                      .or(
                          "account.balance - at_state.prev_balance >="
                              + " at_state.min_activate_amount"))
    ...
    return Db.useDSLContext(
        ctx -> {
          ...
          if (height > 0) {
            where =
                select
                    .where(AT_STATE.HEIGHT.le(height))
                    .and(AT.ID.eq(id))
                    .orderBy(AT_STATE.HEIGHT.desc())
                    .maxRows(1);
          } else {
            where =
                select.where(AT.LATEST.isTrue()).and(AT_STATE.LATEST.isTrue()).and(AT.ID.eq(id));
          }
  ...
  private brs.at.AT createAT(AtRecord at, AtStateRecord atState, int height) {
    return new AT(
        AtApiHelper.getByteArray(at.getId()),
        AtApiHelper.getByteArray(at.getCreatorId()),
        at.getName(),
        at.getDescription(),
        at.getVersion(),
        height,
        brs.at.AT.decompressState(atState.getState()),
        at.getCsize(),
        at.getDsize(),
        at.getCUserStackBytes(),
        at.getCCallStackBytes(),
        at.getCreationHeight(),
        atState.getSleepBetween(),
        atState.getNextHeight(),
        atState.getFreezeWhenSameBalance(),
        atState.getMinActivateAmount(),
        brs.at.AT.decompressState(at.getApCode()));
  }
  ...
  public List<Long> getATsIssuedBy(Long accountId) {
    return Db.useDSLContext(
        ctx -> {
          return ctx.selectFrom(AT)
              .where(AT.LATEST.isTrue())
              .and(AT.CREATOR_ID.eq(accountId))
              .orderBy(AT.CREATION_HEIGHT.desc(), AT.ID.asc())
              .fetch()
              .getValues(AT.ID);
        });
  }
  ...
  @Override
  public Long findTransaction(
      int startHeight, int endHeight, Long atID, int numOfTx, long minAmount) {
    return Db.useDSLContext(
        ctx -> {
          SelectQuery<Record1<Long>> query =
              ctx.select(TRANSACTION.ID)
                  .from(TRANSACTION)
