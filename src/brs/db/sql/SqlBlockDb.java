...
public class SqlBlockDb implements BlockDb {
        ...
        ctx -> {
          try {
            ...
            throw new RuntimeException(
                "Block already in database, id = " + blockId + ", does not pass validation!", e);
          }
        });
  }
  ...
  public long findBlockIdAtHeight(int height) {
    return Db.useDSLContext(
        ctx -> {
          Long id =
              ctx.select(BLOCK.ID).from(BLOCK).where(BLOCK.HEIGHT.eq(height)).fetchOne(BLOCK.ID);
          if (id == null) {
  ...
  public Block findBlockAtHeight(int height) {
    return Db.useDSLContext(
        ctx -> {
          try {
            Block block =
                loadBlock(ctx.selectFrom(BLOCK).where(BLOCK.HEIGHT.eq(height)).fetchAny());
            if (block == null) {
              throw new RuntimeException("Block at height " + height + " not found in database!");
            }
            return block;
    ...
    return Db.useDSLContext(
        ctx -> {
          try {
            ...
            throw new RuntimeException(
                "Last block already in database does not pass validation!", e);
          }
        });
  }
    ...
    return Db.useDSLContext(
        ctx -> {
          try {
            ...
            throw new RuntimeException(
                "Block already in database at timestamp "
                    + timestamp
  ...
  public void saveBlock(DSLContext ctx, Block block) {
    ctx.insertInto(
        ...
        .values(
            ...
            block.getTotalAmountNQT(),
            block.getTotalFeeNQT(),
            block.getPayloadLength(),
            block.getGeneratorPublicKey(),
            block.getPreviousBlockHash(),
            block.getCumulativeDifficulty().toByteArray(),
            block.getBaseTarget(),
            block.getHeight(),
