...
public class SqlAccountStore implements AccountStore {
  ...
  private static final DbKey.LongKeyFactory<Account> accountDbKeyFactory =
      new DbKey.LongKeyFactory<Account>(ACCOUNT.ID) {
        @Override
        public DbKey newKey(Account account) {
          return (DbKey) account.nxtKey;
        }
      };
  private static final DbKey.LongKeyFactory<Account.RewardRecipientAssignment>
      rewardRecipientAssignmentDbKeyFactory =
          new DbKey.LongKeyFactory<Account.RewardRecipientAssignment>(
  ...
  public SqlAccountStore(
      DerivedTableManager derivedTableManager, DBCacheManagerImpl dbCacheManager) {
    rewardRecipientAssignmentTable =
        new VersionedEntitySqlTable<Account.RewardRecipientAssignment>(
            "reward_recip_assign",
          ...
          protected void save(DSLContext ctx, Account.RewardRecipientAssignment assignment) {
            ctx.mergeInto(
                ...
                .values(
                    ...
                    assignment.getPrevRecipientId(),
                    assignment.getRecipientId(),
                    assignment.getFromHeight(),
    ...
    accountAssetTable =
        new VersionedEntitySqlTable<Account.AccountAsset>(
            "account_asset",
          ...
          protected void save(DSLContext ctx, Account.AccountAsset accountAsset) {
            ctx.mergeInto(
                ...
                .values(
                    ...
                    accountAsset.getQuantityQNT(),
                    accountAsset.getUnconfirmedQuantityQNT(),
                    Burst.getBlockchain().getHeight(),
                    true)
                .execute();
          }
    ...
    accountTable =
        new VersionedBatchEntitySqlTable<Account>(
            "account",
              ...
              accountQueries.add(
                  ctx.mergeInto(
                      ...
                      .values(
                          account.getId(),
                          height,
                          account.getCreationHeight(),
                          account.getPublicKey(),
                          account.getKeyHeight(),
                          account.getBalanceNQT(),
                          account.getUnconfirmedBalanceNQT(),
                          account.getForgedBalanceNQT(),
                          account.getName(),
  ...
  private final VersionedEntityTable<Account.RewardRecipientAssignment>
      rewardRecipientAssignmentTable;
  ...
  @Override
  public VersionedEntityTable<Account.RewardRecipientAssignment>
      getRewardRecipientAssignmentTable() {
    return rewardRecipientAssignmentTable;
  }

  @Override
  public DbKey.LongKeyFactory<Account.RewardRecipientAssignment>
  ...
  public long getAllAccountsBalance() {
    return Db.useDSLContext(
        ctx -> {
          return ctx.select(DSL.sum(ACCOUNT.BALANCE))
              .from(ACCOUNT)
              .where(ACCOUNT.ID.ne(0L))
    ...
    return Db.useDSLContext(
        ctx -> {
          return ctx.selectCount()
              .from(ACCOUNT_ASSET)
              .where(ACCOUNT_ASSET.ASSET_ID.eq(assetId))
  ...
  @Override
  public Collection<Account.RewardRecipientAssignment> getAccountsWithRewardRecipient(
      Long recipientId) {
    return getRewardRecipientAssignmentTable()
        .getManyBy(
            getAccountsWithRewardRecipientClause(
                recipientId, Burst.getBlockchain().getHeight() + 1),
            0,
            -1);
  }
  ...
  @Override
  public Collection<Account.AccountAsset> getAssetAccounts(
      long assetId, int height, int from, int to) {
    if (height < 0) {
    ...
    sort.add(ACCOUNT_ASSET.field("quantity", Long.class).desc());
    sort.add(ACCOUNT_ASSET.field("account_id", Long.class).asc());
    return getAccountAssetTable()
        .getManyBy(ACCOUNT_ASSET.ASSET_ID.eq(assetId), height, from, to, sort);
  }
  ...
  public boolean setOrVerify(Account acc, byte[] key, int height) {
    if (acc.getPublicKey() == null) {
      ...
      if (logger.isInfoEnabled()) {
        ...
        logger.info(
            "Account key for {} was already set to a different one at the same height, current"
                + " height is {}, rejecting new key",
            Convert.toUnsignedLong(acc.id),
            height);
      }
      return false;
      ...
      if (Db.isInTransaction()) {
        if (logger.isInfoEnabled()) {
          logger.info(
              "Changing key for account {} at height {}, was previously set to a different one at"
                  + " height {}",
              Convert.toUnsignedLong(acc.id),
              height,
              acc.getKeyHeight());
    ...
    if (logger.isInfoEnabled()) {
      logger.info("DUPLICATE KEY!!!");
      logger.info(
          "Invalid key for account {} at height {}, was already set to a different one at height"
              + " {}",
          Convert.toUnsignedLong(acc.id),
          height,
          acc.getKeyHeight());
  ...
  static class SQLAccountAsset extends Account.AccountAsset {
    SQLAccountAsset(Record rs) {
      super(
          ...
          accountAssetDbKeyFactory.newKey(
              rs.get(ACCOUNT_ASSET.ACCOUNT_ID), rs.get(ACCOUNT_ASSET.ASSET_ID)));
    }
  }
