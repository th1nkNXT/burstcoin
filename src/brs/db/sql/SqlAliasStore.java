...
public class SqlAliasStore implements AliasStore {
  ...
  private static final DbKey.LongKeyFactory<Alias.Offer> offerDbKeyFactory =
      new DbKey.LongKeyFactory<Alias.Offer>(ALIAS_OFFER.ID) {
        @Override
  ...
  public SqlAliasStore(DerivedTableManager derivedTableManager) {
    offerTable =
        new VersionedEntitySqlTable<Alias.Offer>(
            "alias_offer", ALIAS_OFFER, offerDbKeyFactory, derivedTableManager) {
          @Override
    ...
    aliasTable =
        new VersionedEntitySqlTable<Alias>(
            "alias", brs.schema.Tables.ALIAS, aliasDbKeyFactory, derivedTableManager) {
          @Override
  ...
  private static final BurstKey.LongKeyFactory<Alias> aliasDbKeyFactory =
      new DbKey.LongKeyFactory<Alias>(ALIAS.ID) {
  ...
  private class SqlOffer extends Alias.Offer {
    private SqlOffer(Record record) {
      super(
          ...
          Convert.nullToZero(record.get(ALIAS_OFFER.BUYER_ID)),
        ...
        ctx -> {
          ctx.insertInto(
              ...
              .values(
                  offer.getId(),
                  offer.getPriceNQT(),
                  (offer.getBuyerId() == 0 ? null : offer.getBuyerId()),
  ...
  private void saveAlias(DSLContext ctx, Alias alias) {
    ctx.insertInto(ALIAS)
        ...
        .set(ALIAS.HEIGHT, Burst.getBlockchain().getHeight())
        .execute();
  }
  ...
  public Alias getAlias(String aliasName) {
    return aliasTable.getBy(
        brs.schema.Tables.ALIAS.ALIAS_NAME_LOWER.eq(aliasName.toLowerCase(Locale.ENGLISH)));
  }
}
