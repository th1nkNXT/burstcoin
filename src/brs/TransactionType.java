...
public abstract class TransactionType {
  ...
  private static final int BASELINE_FEE_HEIGHT =
      1; // At release time must be less than current block - 1440
  private static final Fee BASELINE_ASSET_ISSUANCE_FEE =
      new Fee(Constants.ASSET_ISSUANCE_FEE_NQT, 0);
  ...
  public static void init(
    ...
    coloredCoinsTypes.put(
        SUBTYPE_COLORED_COINS_ASK_ORDER_PLACEMENT, ColoredCoins.ASK_ORDER_PLACEMENT);
    coloredCoinsTypes.put(
        SUBTYPE_COLORED_COINS_BID_ORDER_PLACEMENT, ColoredCoins.BID_ORDER_PLACEMENT);
    coloredCoinsTypes.put(
        SUBTYPE_COLORED_COINS_ASK_ORDER_CANCELLATION, ColoredCoins.ASK_ORDER_CANCELLATION);
    coloredCoinsTypes.put(
        SUBTYPE_COLORED_COINS_BID_ORDER_CANCELLATION, ColoredCoins.BID_ORDER_CANCELLATION);
    ...
    burstMiningTypes.put(
        SUBTYPE_BURST_MINING_REWARD_RECIPIENT_ASSIGNMENT, BurstMining.REWARD_RECIPIENT_ASSIGNMENT);
    ...
    advancedPaymentTypes.put(
        SUBTYPE_ADVANCED_PAYMENT_ESCROW_CREATION, AdvancedPayment.ESCROW_CREATION);
    ...
    advancedPaymentTypes.put(
        SUBTYPE_ADVANCED_PAYMENT_SUBSCRIPTION_SUBSCRIBE, AdvancedPayment.SUBSCRIPTION_SUBSCRIBE);
    advancedPaymentTypes.put(
        SUBTYPE_ADVANCED_PAYMENT_SUBSCRIPTION_CANCEL, AdvancedPayment.SUBSCRIPTION_CANCEL);
    advancedPaymentTypes.put(
        SUBTYPE_ADVANCED_PAYMENT_SUBSCRIPTION_PAYMENT, AdvancedPayment.SUBSCRIPTION_PAYMENT);
  ...
  public abstract Attachment.AbstractAttachment parseAttachment(
      ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException;

  abstract Attachment.AbstractAttachment parseAttachment(JsonObject attachmentData)
      throws BurstException.NotValidException;

  abstract void validateAttachment(Transaction transaction)
  ...
  public Long calculateTotalAmountNQT(Transaction transaction) {
    return Convert.safeAdd(
        calculateTransactionAmountNQT(transaction), calculateAttachmentTotalAmountNQT(transaction));
  }
  ...
  final void apply(Transaction transaction, Account senderAccount, Account recipientAccount) {
    accountService.addToBalanceNQT(
        senderAccount, -(Convert.safeAdd(transaction.getAmountNQT(), transaction.getFeeNQT())));
    if (transaction.getReferencedTransactionFullHash() != null
        && !Burst.getFluxCapacitor().getValue(FluxValues.SIGNUM, transaction.getHeight())) {
      accountService.addToUnconfirmedBalanceNQT(
          senderAccount, Constants.UNCONFIRMED_POOL_DEPOSIT_NQT);
    }
    if (recipientAccount != null) {
      accountService.addToBalanceAndUnconfirmedBalanceNQT(
          recipientAccount, transaction.getAmountNQT());
    }
    if (logger.isTraceEnabled()) {
      logger.trace(
          "applying transaction - id: {}, type: {}", transaction.getId(), transaction.getType());
    }
    applyAttachment(transaction, senderAccount, recipientAccount);
  }

  abstract void applyAttachment(
  ...
  public void parseAppendices(
      Transaction.Builder builder, int flags, byte version, ByteBuffer buffer)
      throws BurstException.ValidationException {
    int position = 1;
    if ((flags & position) != 0) {
  ...
  public abstract static class Payment extends TransactionType {
    ...
    public static final TransactionType ORDINARY =
        new Payment() {
          ...
          @Override
          public Attachment.EmptyAttachment parseAttachment(
          ...
          @Override
          void validateAttachment(Transaction transaction)
              throws BurstException.ValidationException {
    ...
    public static final TransactionType MULTI_OUT =
        new Payment() {
          ...
          @Override
          public Attachment.PaymentMultiOutCreation parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (!fluxCapacitor.getValue(FluxValues.PRE_POC2, transaction.getHeight())) {
              throw new BurstException.NotCurrentlyValidException(
                  "Multi Out Payments are not allowed before the Pre POC2 block");
            }

            Attachment.PaymentMultiOutCreation attachment =
                (Attachment.PaymentMultiOutCreation) transaction.getAttachment();
          ...
          @Override
          final void applyAttachment(
              Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.PaymentMultiOutCreation attachment =
                (Attachment.PaymentMultiOutCreation) transaction.getAttachment();
            for (List<Long> recipient : attachment.getRecipients()) {
              accountService.addToBalanceAndUnconfirmedBalanceNQT(
                  accountService.getOrAddAccount(recipient.get(0)), recipient.get(1));
            }
          }
    ...
    public static final TransactionType MULTI_SAME_OUT =
        new Payment() {
          ...
          @Override
          public Attachment.PaymentMultiSameOutCreation parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (!fluxCapacitor.getValue(FluxValues.PRE_POC2, transaction.getHeight())) {
              throw new BurstException.NotCurrentlyValidException(
                  "Multi Same Out Payments are not allowed before the Pre POC2 block");
            }

            Attachment.PaymentMultiSameOutCreation attachment =
                (Attachment.PaymentMultiSameOutCreation) transaction.getAttachment();
          ...
          final void applyAttachment(
            ...
            final long amountNQT =
                Convert.safeDivide(transaction.getAmountNQT(), attachment.getRecipients().size());
            attachment
                .getRecipients()
                .forEach(
                    a ->
                        accountService.addToBalanceAndUnconfirmedBalanceNQT(
                            accountService.getOrAddAccount(a), amountNQT));
          }
  ...
  public abstract static class Messaging extends TransactionType {
    ...
    public static final TransactionType ALIAS_ASSIGNMENT =
        new Messaging() {
          ...
          @Override
          public Attachment.MessagingAliasAssignment parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.MessagingAliasAssignment attachment =
                (Attachment.MessagingAliasAssignment) transaction.getAttachment();
            return new TransactionDuplicationKey(
                Messaging.ALIAS_ASSIGNMENT, attachment.getAliasName().toLowerCase(Locale.ENGLISH));
          }
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (!TextUtils.isInAlphabet(attachment.getAliasName())) {
              throw new BurstException.NotValidException(
                  "Invalid alias name: " + attachment.getAliasName());
            }
    ...
    public static final TransactionType ALIAS_SELL =
        new Messaging() {
          ...
          @Override
          public Attachment.MessagingAliasSell parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          @Override
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.MessagingAliasSell attachment =
                (Attachment.MessagingAliasSell) transaction.getAttachment();
          ...
          void validateAttachment(Transaction transaction)
              throws BurstException.ValidationException {
            if (!fluxCapacitor.getValue(
                FluxValues.DIGITAL_GOODS_STORE, blockchain.getLastBlock().getHeight())) {
              throw new BurstException.NotYetEnabledException(
                  "Alias transfer not yet enabled at height "
                      + blockchain.getLastBlock().getHeight());
            }
            if (transaction.getAmountNQT() != 0) {
              throw new BurstException.NotValidException(
                  "Invalid sell alias transaction: "
    ...
    public static final TransactionType ALIAS_BUY =
        new Messaging() {
          ...
          @Override
          public Attachment.MessagingAliasBuy parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          void applyAttachment(
            ...
            final String aliasName = attachment.getAliasName();
            aliasService.changeOwner(
                transaction.getSenderId(), aliasName, transaction.getBlockTimestamp());
          }

          @Override
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.MessagingAliasBuy attachment =
                (Attachment.MessagingAliasBuy) transaction.getAttachment();
            // not a bug, uniqueness is based on Messaging.ALIAS_ASSIGNMENT
            return new TransactionDuplicationKey(
                Messaging.ALIAS_ASSIGNMENT, attachment.getAliasName().toLowerCase(Locale.ENGLISH));
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (offer == null) {
              throw new BurstException.NotCurrentlyValidException(
                  "Alias is not for sale: " + aliasName);
            }
            if (transaction.getAmountNQT() < offer.getPriceNQT()) {
              String msg =
                  "Price is too low for: "
                      + aliasName
                      + " ("
    ...
    public static final Messaging ACCOUNT_INFO =
        new Messaging() {
          ...
          @Override
          public Attachment.MessagingAccountInfo parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (Convert.toBytes(attachment.getName()).length > Constants.MAX_ACCOUNT_NAME_LENGTH
                || Convert.toBytes(attachment.getDescription()).length
          ...
          void applyAttachment(
            ...
            accountService.setAccountInfo(
                senderAccount, attachment.getName(), attachment.getDescription());
          }
  ...
  public abstract static class ColoredCoins extends TransactionType {
    ...
    public static final TransactionType ASSET_ISSUANCE =
        new ColoredCoins() {
          ...
          @Override
          public Attachment.ColoredCoinsAssetIssuance parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          void applyAttachment(
            ...
            accountService.addToAssetAndUnconfirmedAssetBalanceQNT(
                senderAccount, assetId, attachment.getQuantityQNT());
          }
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (!TextUtils.isInAlphabet(attachment.getName())) {
              throw new BurstException.NotValidException(
                  "Invalid asset name: " + attachment.getName());
            }
          }
    ...
    public static final TransactionType ASSET_TRANSFER =
        new ColoredCoins() {
          ...
          @Override
          public Attachment.ColoredCoinsAssetTransfer parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            ...
            long unconfirmedAssetBalance =
                accountService.getUnconfirmedAssetBalanceQNT(
                    senderAccount, attachment.getAssetId());
            if (unconfirmedAssetBalance >= 0
                && unconfirmedAssetBalance >= attachment.getQuantityQNT()) {
              accountService.addToUnconfirmedAssetBalanceQNT(
                  senderAccount, attachment.getAssetId(), -attachment.getQuantityQNT());
              return true;
            }
            return false;
          }
          ...
          void applyAttachment(
            ...
            Attachment.ColoredCoinsAssetTransfer attachment =
                (Attachment.ColoredCoinsAssetTransfer) transaction.getAttachment();
            accountService.addToAssetBalanceQNT(
                senderAccount, attachment.getAssetId(), -attachment.getQuantityQNT());
            accountService.addToAssetAndUnconfirmedAssetBalanceQNT(
                recipientAccount, attachment.getAssetId(), attachment.getQuantityQNT());
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (transaction.getAmountNQT() != 0
                || attachment.getComment() != null
                    && attachment.getComment().length()
                        > Constants.MAX_ASSET_TRANSFER_COMMENT_LENGTH
                || attachment.getAssetId() == 0) {
              throw new BurstException.NotValidException(
                  "Invalid asset transfer amount or comment: "
                      + JSON.toJsonString(attachment.getJsonObject()));
            }
            ...
            if (attachment.getQuantityQNT() <= 0
              ...
              throw new BurstException.NotValidException(
                  "Invalid asset transfer asset or quantity: "
                      + JSON.toJsonString(attachment.getJsonObject()));
            }
            if (asset == null) {
              throw new BurstException.NotCurrentlyValidException(
                  "Asset "
                      + Convert.toUnsignedLong(attachment.getAssetId())
                      + " does not exist yet");
            }
          }
    ...
    abstract static class ColoredCoinsOrderPlacement extends ColoredCoins {
      ...
      @Override
      final void validateAttachment(Transaction transaction)
          throws BurstException.ValidationException {
        Attachment.ColoredCoinsOrderPlacement attachment =
            (Attachment.ColoredCoinsOrderPlacement) transaction.getAttachment();
        if (attachment.getPriceNQT() <= 0
            || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
            || attachment.getAssetId() == 0) {
          throw new BurstException.NotValidException(
        ...
        if (attachment.getQuantityQNT() <= 0
            || (asset != null && attachment.getQuantityQNT() > asset.getQuantityQNT())) {
          throw new BurstException.NotValidException(
              "Invalid asset order placement asset or quantity: "
                  + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (asset == null) {
          throw new BurstException.NotCurrentlyValidException(
    ...
    public static final TransactionType ASK_ORDER_PLACEMENT =
        new ColoredCoinsOrderPlacement() {
          ...
          @Override
          public Attachment.ColoredCoinsAskOrderPlacement parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
    ...
    public static final TransactionType BID_ORDER_PLACEMENT =
        new ColoredCoinsOrderPlacement() {
          ...
          @Override
          public Attachment.ColoredCoinsBidOrderPlacement parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
    ...
    public static final TransactionType ASK_ORDER_CANCELLATION =
        new ColoredCoinsOrderCancellation() {
          ...
          @Override
          public Attachment.ColoredCoinsAskOrderCancellation parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          void applyAttachment(
            ...
            if (order != null) {
              accountService.addToUnconfirmedAssetBalanceQNT(
                  senderAccount, order.getAssetId(), order.getQuantityQNT());
            }
          }
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (ask.getAccountId() != transaction.getSenderId()) {
              throw new BurstException.NotValidException(
                  "Order "
                      + Convert.toUnsignedLong(attachment.getOrderId())
    ...
    public static final TransactionType BID_ORDER_CANCELLATION =
        new ColoredCoinsOrderCancellation() {
          ...
          @Override
          public Attachment.ColoredCoinsBidOrderCancellation parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          void applyAttachment(
            ...
            if (order != null) {
              accountService.addToUnconfirmedBalanceNQT(
                  senderAccount, Convert.safeMultiply(order.getQuantityQNT(), order.getPriceNQT()));
            }
          }
  ...
  public abstract static class DigitalGoods extends TransactionType {
    ...
    final void validateAttachment(Transaction transaction)
      ...
      if (!fluxCapacitor.getValue(
          FluxValues.DIGITAL_GOODS_STORE, blockchain.getLastBlock().getHeight())) {
        throw new BurstException.NotYetEnabledException(
            "Digital goods listing not yet enabled at height "
                + blockchain.getLastBlock().getHeight());
      }
      if (transaction.getAmountNQT() != 0) {
    ...
    abstract void doValidateAttachment(Transaction transaction)
        throws BurstException.ValidationException;
    ...
    public static final TransactionType LISTING =
        new DigitalGoods() {
          ...
          @Override
          public Attachment.DigitalGoodsListing parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          void applyAttachment(
              Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.DigitalGoodsListing attachment =
                (Attachment.DigitalGoodsListing) transaction.getAttachment();
          ...
          void doValidateAttachment(Transaction transaction)
            ...
            if (attachment.getName().isEmpty()
                ...
                || attachment.getDescription().length()
    ...
    public static final TransactionType DELISTING =
        new DigitalGoods() {
          ...
          @Override
          public Attachment.DigitalGoodsDelisting parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          void applyAttachment(
              Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.DigitalGoodsDelisting attachment =
                (Attachment.DigitalGoodsDelisting) transaction.getAttachment();
          ...
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.DigitalGoodsDelisting attachment =
                (Attachment.DigitalGoodsDelisting) transaction.getAttachment();
            return new TransactionDuplicationKey(
                DigitalGoods.DELISTING, Convert.toUnsignedLong(attachment.getGoodsId()));
          }
    ...
    public static final TransactionType PRICE_CHANGE =
        new DigitalGoods() {
          ...
          @Override
          public Attachment.DigitalGoodsPriceChange parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          void applyAttachment(
              Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.DigitalGoodsPriceChange attachment =
                (Attachment.DigitalGoodsPriceChange) transaction.getAttachment();
          ...
          void doValidateAttachment(Transaction transaction)
            ...
            if (attachment.getPriceNQT() <= 0
              ...
              throw new BurstException.NotValidException(
                  "Invalid digital goods price change: "
                      + JSON.toJsonString(attachment.getJsonObject()));
            }
            if (goods == null || goods.isDelisted()) {
              throw new BurstException.NotCurrentlyValidException(
    ...
    public static final TransactionType QUANTITY_CHANGE =
        new DigitalGoods() {
          ...
          @Override
          public Attachment.DigitalGoodsQuantityChange parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          void applyAttachment(
              Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.DigitalGoodsQuantityChange attachment =
                (Attachment.DigitalGoodsQuantityChange) transaction.getAttachment();
            dgsGoodsStoreService.changeQuantity(
                attachment.getGoodsId(), attachment.getDeltaQuantity(), false);
          }
          ...
          void doValidateAttachment(Transaction transaction)
            ...
            if (attachment.getDeltaQuantity() < -Constants.MAX_DGS_LISTING_QUANTITY
                ...
                || (goods != null && transaction.getSenderId() != goods.getSellerId())) {
              throw new BurstException.NotValidException(
                  "Invalid digital goods quantity change: "
                      + JSON.toJsonString(attachment.getJsonObject()));
            }
            if (goods == null || goods.isDelisted()) {
              throw new BurstException.NotCurrentlyValidException(
    ...
    public static final TransactionType PURCHASE =
        new DigitalGoods() {
          ...
          @Override
          public Attachment.DigitalGoodsPurchase parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          @Override
          public Long calculateAttachmentTotalAmountNQT(Transaction transaction) {
            Attachment.DigitalGoodsPurchase attachment =
                (Attachment.DigitalGoodsPurchase) transaction.getAttachment();
          ...
          void undoAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            accountService.addToUnconfirmedBalanceNQT(
                senderAccount, calculateAttachmentTotalAmountNQT(transaction));
          }
          ...
          void doValidateAttachment(Transaction transaction)
            ...
            if (attachment.getQuantity() <= 0
              ...
              throw new BurstException.NotValidException(
                  "Invalid digital goods purchase: "
                      + JSON.toJsonString(attachment.getJsonObject()));
            }
            if (transaction.getEncryptedMessage() != null
                && !transaction.getEncryptedMessage().isText()) {
            ...
            if (attachment.getQuantity() > goods.getQuantity()
              ...
              throw new BurstException.NotCurrentlyValidException(
                  "Goods price or quantity changed: "
                      + JSON.toJsonString(attachment.getJsonObject()));
            }
            if (attachment.getDeliveryDeadlineTimestamp()
                <= blockchain.getLastBlock().getTimestamp()) {
              throw new BurstException.NotCurrentlyValidException(
                  "Delivery deadline has already expired: "
                      + attachment.getDeliveryDeadlineTimestamp());
            }
          }
    ...
    public static final TransactionType DELIVERY =
        new DigitalGoods() {
          ...
          @Override
          public Attachment.DigitalGoodsDelivery parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          void applyAttachment(
              Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.DigitalGoodsDelivery attachment =
                (Attachment.DigitalGoodsDelivery) transaction.getAttachment();
          ...
          void doValidateAttachment(Transaction transaction)
            ...
            DigitalGoodsStore.Purchase purchase =
                dgsGoodsStoreService.getPendingPurchase(attachment.getPurchaseId());
            if (attachment.getGoods().getData().length > Constants.MAX_DGS_GOODS_LENGTH
                ...
                || attachment.getDiscountNQT() > Constants.MAX_BALANCE_NQT
                || (purchase != null
                    && (purchase.getBuyerId() != transaction.getRecipientId()
                        || transaction.getSenderId() != purchase.getSellerId()
                        || attachment.getDiscountNQT()
                            > Convert.safeMultiply(
                                purchase.getPriceNQT(), purchase.getQuantity())))) {
              throw new BurstException.NotValidException(
                  "Invalid digital goods delivery: "
                      + JSON.toJsonString(attachment.getJsonObject()));
            }
          ...
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.DigitalGoodsDelivery attachment =
                (Attachment.DigitalGoodsDelivery) transaction.getAttachment();
            return new TransactionDuplicationKey(
                DigitalGoods.DELIVERY, Convert.toUnsignedLong(attachment.getPurchaseId()));
          }
    ...
    public static final TransactionType FEEDBACK =
        new DigitalGoods() {
          ...
          @Override
          public Attachment.DigitalGoodsFeedback parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          void applyAttachment(
              Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.DigitalGoodsFeedback attachment =
                (Attachment.DigitalGoodsFeedback) transaction.getAttachment();
            dgsGoodsStoreService.feedback(
                attachment.getPurchaseId(),
                transaction.getEncryptedMessage(),
          ...
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.DigitalGoodsFeedback attachment =
                (Attachment.DigitalGoodsFeedback) transaction.getAttachment();
            return new TransactionDuplicationKey(
                DigitalGoods.FEEDBACK, Convert.toUnsignedLong(attachment.getPurchaseId()));
          }
    ...
    public static final TransactionType REFUND =
        new DigitalGoods() {
          ...
          @Override
          public Attachment.DigitalGoodsRefund parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          @Override
          public Long calculateAttachmentTotalAmountNQT(Transaction transaction) {
            Attachment.DigitalGoodsRefund attachment =
                (Attachment.DigitalGoodsRefund) transaction.getAttachment();
          ...
          void doValidateAttachment(Transaction transaction)
            ...
            DigitalGoodsStore.Purchase purchase =
                dgsGoodsStoreService.getPurchase(attachment.getPurchaseId());
            if (attachment.getRefundNQT() < 0
                || attachment.getRefundNQT() > Constants.MAX_BALANCE_NQT
                || (purchase != null
                    && (purchase.getBuyerId() != transaction.getRecipientId()
                        || transaction.getSenderId() != purchase.getSellerId()))) {
              throw new BurstException.NotValidException(
          ...
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.DigitalGoodsRefund attachment =
                (Attachment.DigitalGoodsRefund) transaction.getAttachment();
            return new TransactionDuplicationKey(
                DigitalGoods.REFUND, Convert.toUnsignedLong(attachment.getPurchaseId()));
          }
  ...
  public abstract static class AccountControl extends TransactionType {
    ...
    public static final TransactionType EFFECTIVE_BALANCE_LEASING =
        new AccountControl() {
          ...
          @Override
          public Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
            return new Attachment.AccountControlEffectiveBalanceLeasing(buffer, transactionVersion);
          }

          @Override
          Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(
          ...
          void applyAttachment(
              Transaction transaction, Account senderAccount, Account recipientAccount) {
            // This tx type is actually used before block 10k on mainnet, so we cannot actually
            // remove it
          }
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (Burst.getFluxCapacitor().getValue(FluxValues.SODIUM))
              throw new BurstException.NotCurrentlyValidException(
                  "Effective Balance Leasing disabled after Sodium HF");
  ...
  public abstract static class BurstMining extends TransactionType {
    ...
    public static final TransactionType REWARD_RECIPIENT_ASSIGNMENT =
        new BurstMining() {
          ...
          @Override
          public Attachment.BurstMiningRewardRecipientAssignment parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
            return new Attachment.BurstMiningRewardRecipientAssignment(buffer, transactionVersion);
          }

          @Override
          Attachment.BurstMiningRewardRecipientAssignment parseAttachment(
              JsonObject attachmentData) {
          ...
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            if (!fluxCapacitor.getValue(FluxValues.DIGITAL_GOODS_STORE)) {
              return TransactionDuplicationKey
                  .IS_NEVER_DUPLICATE; // sync fails after 7007 without this
            }

            return new TransactionDuplicationKey(
                BurstMining.REWARD_RECIPIENT_ASSIGNMENT,
                Convert.toUnsignedLong(transaction.getSenderId()));
          }
          ...
          void validateAttachment(Transaction transaction)
            ...
            Account.RewardRecipientAssignment rewardAssignment =
                accountService.getRewardRecipientAssignment(sender);
            ...
            if (fluxCapacitor.getValue(FluxValues.PRE_POC2)) {
              if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() < FEE_QUANT) {
                throw new BurstException.NotValidException(
                    "Reward recipient assignment transaction must have 0 send amount and at least"
                        + " minimum fee: "
                        + JSON.toJsonString(transaction.getJsonObject()));
              }
            } else {
              if (transaction.getAmountNQT() != 0
                  || transaction.getFeeNQT() != Constants.ONE_BURST) {
                throw new BurstException.NotValidException(
            ...
            if (!Burst.getFluxCapacitor().getValue(FluxValues.REWARD_RECIPIENT_ENABLE, height)) {
              throw new BurstException.NotCurrentlyValidException(
                  "Reward recipient assignment not allowed before block "
                      + Burst.getFluxCapacitor()
                          .getStartingHeight(FluxValues.REWARD_RECIPIENT_ENABLE));
            }
          }
    ...
    public static final TransactionType COMMITMENT_ADD =
        new BurstMining() {
          ...
          @Override
          public Attachment.CommitmentAdd parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            ...
            CommitmentAdd commitmentAdd = (CommitmentAdd) transaction.getAttachment();
            Long totalAmountNQT = commitmentAdd.getAmountNQT();
            if (totalAmountNQT < 0L) return false;
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (!Burst.getFluxCapacitor().getValue(FluxValues.SIGNUM, height)) {
              throw new BurstException.NotCurrentlyValidException(
                  "Add commitment not allowed before block "
                      + Burst.getFluxCapacitor().getStartingHeight(FluxValues.SIGNUM));
            }
          }
    ...
    public static final TransactionType COMMITMENT_REMOVE =
        new BurstMining() {
          ...
          @Override
          public Attachment.CommitmentRemove parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          @Override
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            // All commitment remove transactions from the same account are considered duplicates,
            // so just the one with highest fee is kept
            return new TransactionDuplicationKey(
                BurstMining.COMMITMENT_REMOVE, Convert.toUnsignedLong(transaction.getSenderId()));
          }
  ...
  public abstract static class AdvancedPayment extends TransactionType {
    ...
    public static final TransactionType ESCROW_CREATION =
        new AdvancedPayment() {
          ...
          @Override
          public Attachment.AdvancedPaymentEscrowCreation parseAttachment(
              ByteBuffer buffer, byte transactionVersion) throws BurstException.NotValidException {
          ...
          public Long calculateAttachmentTotalAmountNQT(Transaction transaction) {
            ...
            return Convert.safeAdd(
                attachment.getAmountNQT(),
                Convert.safeMultiply(attachment.getTotalSigners(), Constants.ONE_BURST));
          }
          ...
          final void applyAttachment(
            ...
            signers.forEach(
                signer ->
                    accountService.addToBalanceAndUnconfirmedBalanceNQT(
                        accountService.getOrAddAccount(signer), Constants.ONE_BURST));
            escrowService.addEscrowTransaction(
                senderAccount,
                recipientAccount,
                transaction.getId(),
                attachment.getAmountNQT(),
          ...
          void validateAttachment(Transaction transaction)
            ...
            Attachment.AdvancedPaymentEscrowCreation attachment =
                (Attachment.AdvancedPaymentEscrowCreation) transaction.getAttachment();
            Long totalAmountNQT =
                Convert.safeAdd(attachment.getAmountNQT(), transaction.getFeeNQT());
            if (transaction.getSenderId() == transaction.getRecipientId()) {
              throw new BurstException.NotValidException(
                  "Escrow must have different sender and recipient");
            }
            totalAmountNQT =
                Convert.safeAdd(totalAmountNQT, attachment.getTotalSigners() * Constants.ONE_BURST);
            if (transaction.getAmountNQT() != 0) {
              throw new BurstException.NotValidException(
                  "Transaction sent amount must be 0 for escrow");
            }
            ...
            if (transaction.getFeeNQT() < Constants.ONE_BURST) {
              throw new BurstException.NotValidException(
                  "Escrow transaction must have a fee at least 1 burst");
            }
            ...
            if (attachment.getRequiredSigners() > attachment.getTotalSigners()) {
              throw new BurstException.NotValidException(
                  "Cannot have more required than signers on escrow");
            }
            if (attachment.getTotalSigners() < 1 || attachment.getTotalSigners() > 10) {
              throw new BurstException.NotValidException(
                  "Escrow transaction requires 1 - 10 signers");
            }
            if (attachment.getDeadline() < 1
                || attachment.getDeadline() > 7776000) { // max deadline 3 months
              throw new BurstException.NotValidException(
                  "Escrow deadline must be 1 - 7776000 seconds");
            }
            ...
            if (attachment.getSigners().contains(transaction.getSenderId())
              ...
              throw new BurstException.NotValidException(
                  "Escrow sender and recipient cannot be signers");
            }
    ...
    public static final TransactionType ESCROW_SIGN =
        new AdvancedPayment() {
          ...
          @Override
          public Attachment.AdvancedPaymentEscrowSign parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.AdvancedPaymentEscrowSign attachment =
                (Attachment.AdvancedPaymentEscrowSign) transaction.getAttachment();
            String uniqueString =
                Convert.toUnsignedLong(attachment.getEscrowId())
                    + ":"
                    + Convert.toUnsignedLong(transaction.getSenderId());
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() != Constants.ONE_BURST) {
              throw new BurstException.NotValidException(
                  "Escrow signing must have amount 0 and fee of 1");
            }
            if (attachment.getEscrowId() == null || attachment.getDecision() == null) {
              throw new BurstException.NotValidException(
                  "Escrow signing requires escrow id and decision set");
            }
            Escrow escrow = escrowService.getEscrowTransaction(attachment.getEscrowId());
            if (escrow == null) {
            ...
            if (!escrowService.isIdSigner(transaction.getSenderId(), escrow)
              ...
              throw new BurstException.NotValidException(
                  "Sender is not a participant in specified escrow");
            }
    ...
    public static final TransactionType ESCROW_RESULT =
        new AdvancedPayment() {
          ...
          @Override
          public Attachment.AdvancedPaymentEscrowResult parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
    ...
    public static final TransactionType SUBSCRIPTION_SUBSCRIBE =
        new AdvancedPayment() {
          ...
          @Override
          public Attachment.AdvancedPaymentSubscriptionSubscribe parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
            return new Attachment.AdvancedPaymentSubscriptionSubscribe(buffer, transactionVersion);
          }

          @Override
          Attachment.AdvancedPaymentSubscriptionSubscribe parseAttachment(
              JsonObject attachmentData) {
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (transaction.getAmountNQT() < Constants.ONE_BURST
                || transaction.getAmountNQT() > Constants.MAX_BALANCE_NQT) {
              throw new BurstException.NotValidException(
                  "Subscriptions must be at least one burst");
            }
            if (transaction.getSenderId() == transaction.getRecipientId()) {
              throw new BurstException.NotValidException(
                  "Cannot create subscription to same address");
            }
    ...
    public static final TransactionType SUBSCRIPTION_CANCEL =
        new AdvancedPayment() {
          ...
          @Override
          public Attachment.AdvancedPaymentSubscriptionCancel parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
          ...
          public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
            Attachment.AdvancedPaymentSubscriptionCancel attachment =
                (Attachment.AdvancedPaymentSubscriptionCancel) transaction.getAttachment();
            return new TransactionDuplicationKey(
                AdvancedPayment.SUBSCRIPTION_CANCEL,
          ...
          void validateAttachment(Transaction transaction)
            ...
            if (attachment.getSubscriptionId() == null) {
              throw new BurstException.NotValidException(
                  "Subscription cancel must include subscription id");
            }

            Subscription subscription =
                subscriptionService.getSubscription(attachment.getSubscriptionId());
            ...
            if (!subscription.getSenderId().equals(transaction.getSenderId())
              ...
              throw new BurstException.NotValidException(
                  "Subscription cancel can only be done by participants");
            }

            if (!subscriptionService.isEnabled()) {
              throw new BurstException.NotYetEnabledException(
                  "Subscription cancel not yet enabled");
            }
          }
    ...
    public static final TransactionType SUBSCRIPTION_PAYMENT =
        new AdvancedPayment() {
          ...
          @Override
          public Attachment.AdvancedPaymentSubscriptionPayment parseAttachment(
              ByteBuffer buffer, byte transactionVersion) {
  ...
  public abstract static class AutomatedTransactions extends TransactionType {
    ...
    public static final TransactionType AUTOMATED_TRANSACTION_CREATION =
        new AutomatedTransactions() {
          ...
          void doValidateAttachment(Transaction transaction) throws ValidationException {
            if (!fluxCapacitor.getValue(
                FluxValues.AUTOMATED_TRANSACTION_BLOCK, blockchain.getLastBlock().getHeight())) {
              throw new BurstException.NotYetEnabledException(
                  "Automated Transactions not yet enabled at height "
                      + blockchain.getLastBlock().getHeight());
            }
            if (transaction.getSignature() != null
                && accountService.getAccount(transaction.getId()) != null) {
            ...
            try {
              totalPages =
                  AtController.checkCreationBytes(
                      attachment.getCreationBytes(), blockchain.getHeight());
            } catch (AtException e) {
              throw new BurstException.NotCurrentlyValidException("Invalid AT creation bytes", e);
            }
            long requiredFee =
                totalPages * AtConstants.getInstance().costPerPage(transaction.getHeight());
            if (transaction.getFeeNQT() < requiredFee) {
              throw new BurstException.NotValidException(
                  "Insufficient fee for AT creation. Minimum: "
                      + Convert.toUnsignedLong(requiredFee / Constants.ONE_BURST));
            }
            if (fluxCapacitor.getValue(FluxValues.AT_FIX_BLOCK_3)) {
              if (attachment.getName().length() > Constants.MAX_AUTOMATED_TRANSACTION_NAME_LENGTH) {
                throw new BurstException.NotValidException(
                    "Name of automated transaction over size limit");
              }
              if (attachment.getDescription().length()
    ...
    public static final TransactionType AT_PAYMENT =
        new AutomatedTransactions() {
          @Override
          public final byte getSubtype() {
  ...
  public long minimumFeeNQT(int height, int appendagesSize) {
    ...
    return Convert.safeAdd(
        fee.getConstantFee(), Convert.safeMultiply(appendagesSize, fee.getAppendagesFee()));
  }

  protected Fee getBaselineFee(int height) {
    return new Fee(
        (fluxCapacitor.getValue(FluxValues.PRE_POC2, height) ? FEE_QUANT : ONE_BURST), 0);
  }
