...
public final class DebugTrace {
  ...
  static void init(
    ...
    final DebugTrace debugTrace =
        addDebugTrace(accountIds, logName, blockchainProcessor, accountService, assetExchange);
    blockchainProcessor.addListener(
        block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    logger.debug(
        "Debug tracing of "
            + (accountIdStrings.contains("*") ? "ALL" : String.valueOf(accountIds.size()))
            + " accounts enabled");
  }

  private static DebugTrace addDebugTrace(
      Set<Long> accountIds,
      String logName,
      BlockchainProcessor blockchainProcessor,
      AccountService accountService,
    ...
    if (LOG_UNCONFIRMED) {
      accountService.addListener(
          account -> debugTrace.trace(account, true), Account.Event.UNCONFIRMED_BALANCE);
    }
    accountService.addAssetListener(
        accountAsset -> debugTrace.trace(accountAsset, false), Account.Event.ASSET_BALANCE);
    if (LOG_UNCONFIRMED) {
      accountService.addAssetListener(
          accountAsset -> debugTrace.trace(accountAsset, true),
          Account.Event.UNCONFIRMED_ASSET_BALANCE);
    }
    blockchainProcessor.addListener(
        debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
    blockchainProcessor.addListener(
        debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
    return debugTrace;
  }
  ...
  private void resetLog() {
    ...
    try {
      log =
          new PrintWriter(
              (new BufferedWriter(new OutputStreamWriter(new FileOutputStream(logName)))), true);
    } catch (IOException e) {
  ...
  private boolean include(Attachment attachment) {
    if (attachment instanceof Attachment.DigitalGoodsPurchase) {
      long sellerId =
          dgsGoodsStoreService
              .getGoods(((Attachment.DigitalGoodsPurchase) attachment).getGoodsId())
              .getSellerId();
      return include(sellerId);
    } else if (attachment instanceof Attachment.DigitalGoodsDelivery) {
      long buyerId =
          dgsGoodsStoreService
              .getPurchase(((Attachment.DigitalGoodsDelivery) attachment).getPurchaseId())
              .getBuyerId();
      return include(buyerId);
    } else if (attachment instanceof Attachment.DigitalGoodsRefund) {
      long buyerId =
          dgsGoodsStoreService
              .getPurchase(((Attachment.DigitalGoodsRefund) attachment).getPurchaseId())
              .getBuyerId();
      return include(buyerId);
    }
    return false;
  ...
  private Map<String, String> getValues(long accountId, boolean unconfirmed) {
    ...
    map.put(
        "unconfirmed balance",
        String.valueOf(account != null ? account.getUnconfirmedBalanceNQT() : 0));
    map.put("timestamp", String.valueOf(Burst.getBlockchain().getLastBlock().getTimestamp()));
    map.put("height", String.valueOf(Burst.getBlockchain().getHeight()));
  ...
  private Map<String, String> getValues(long accountId, Trade trade, boolean isAsk) {
    ...
    map.put(
        "trade quantity", String.valueOf(isAsk ? -trade.getQuantityQNT() : trade.getQuantityQNT()));
  ...
  private Map<String, String> getValues(
      long accountId, Transaction transaction, boolean isRecipient) {
  ...
  private Map<String, String> getValues(
      long accountId, Account.AccountAsset accountAsset, boolean unconfirmed) {
    Map<String, String> map = new HashMap<>();
    map.put("account", Convert.toUnsignedLong(accountId));
    map.put("asset", Convert.toUnsignedLong(accountAsset.getAssetId()));
    if (unconfirmed) {
      map.put(
          "unconfirmed asset balance", String.valueOf(accountAsset.getUnconfirmedQuantityQNT()));
    } else {
  ...
  private Map<String, String> getValues(
      long accountId, Transaction transaction, Attachment attachment, boolean isRecipient) {
    Map<String, String> map = getValues(accountId, false);
    if (attachment instanceof Attachment.ColoredCoinsOrderPlacement) {
      if (isRecipient) {
        return Collections.emptyMap();
      }
      Attachment.ColoredCoinsOrderPlacement orderPlacement =
      ...
      BigInteger orderCost =
          BigInteger.valueOf(orderPlacement.getPriceNQT())
      ...
      Attachment.ColoredCoinsAssetIssuance assetIssuance =
      ...
      Attachment.ColoredCoinsAssetTransfer assetTransfer =
      ...
      Attachment.DigitalGoodsPurchase purchase =
      ...
      Attachment.DigitalGoodsDelivery delivery =
          (Attachment.DigitalGoodsDelivery) transaction.getAttachment();
      DigitalGoodsStore.Purchase purchase =
          dgsGoodsStoreService.getPurchase(delivery.getPurchaseId());
      if (isRecipient) {
      ...
      Attachment.DigitalGoodsRefund refund =
          (Attachment.DigitalGoodsRefund) transaction.getAttachment();
      if (isRecipient) {
        map =
            getValues(dgsGoodsStoreService.getPurchase(refund.getPurchaseId()).getBuyerId(), false);
      }
