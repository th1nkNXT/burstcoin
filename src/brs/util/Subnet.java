...
public class Subnet {
  ...
  private Subnet(final InetAddress subnetAddress, final int bits) {
    ...
    this.bigMask =
        BigInteger.valueOf(-1)
            .shiftLeft(this.bytesSubnetCount * 8 - bits); // mask = -1 << 32 - bits
  ...
  private Subnet(final InetAddress subnetAddress, final InetAddress mask) {
    ...
    this.bigMask =
        null == mask
            ? BigInteger.valueOf(-1)
            : new BigInteger(mask.getAddress()); // no mask given case is handled here.
    this.bigSubnetMasked = new BigInteger(subnetAddress.getAddress()).and(this.bigMask);
  }
   ...
   *
   * @param subnetMask format: "192.168.0.0/24" or "192.168.0.0/255.255.255.0" or single address or
  ...
  public static Subnet createInstance(final String subnetMask) throws UnknownHostException {
    ...
    if (2 > stringArr.length) return new Subnet(InetAddress.getByName(stringArr[0]), null);
