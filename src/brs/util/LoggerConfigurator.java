...
public final class LoggerConfigurator {
   ...
   *
   * <p>The existing Java logging configuration will be used if the Java logger has already been
   * initialized. Otherwise, we will configure our own log manager and log handlers. The
   * conf/logging-default.properties and conf/logging.properties configuration files will be used.
   * Entries in logging.properties will override entries in logging-default.properties.
   */
  public static void init() {
    ...
    if (!(LogManager.getLogManager() instanceof BurstLogManager)) {
      System.setProperty(
          managerPackage, (oldManager != null ? oldManager : "java.util.logging.LogManager"));
    }
    if (!Boolean.getBoolean("brs.doNotConfigureLogging")) {
      try {
        ...
        try (InputStream is =
            new FileInputStream(new File(Burst.CONF_FOLDER, "logging-default.properties"))) {
          if (is != null) {
        ...
        try (InputStream is =
            new FileInputStream(new File(Burst.CONF_FOLDER, "logging.properties"))) {
          if (is != null) {
            loggingProperties.load(is);
            foundProperties = true;
          }
