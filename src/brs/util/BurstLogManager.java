...
class BurstLogManager extends LogManager {
   ...
   *
   * <p>We will let the Java LogManager create its shutdown hook so that the shutdown context will
   * be set up properly. However, we will intercept the reset() method so we can delay the actual
   * shutdown until we are done terminating the Burst processes.
   */
   ...
   *
   * <p>This method is called to reset the log handlers. We will forward the call during logging
   * reconfiguration but will ignore it otherwise. This allows us to continue to use logging
   * facilities during Burst shutdown.
   */
  @Override
  public void reset() {
    if (loggingReconfiguration.get()) super.reset();
  }

  /** Burst shutdown is now complete, so call LogManager.reset() to terminate the log handlers. */
  void burstShutdown() {
    super.reset();
  }
}
