...
public class FluxCapacitorImpl implements FluxCapacitor {
  ...
  private final Map<FluxValue<?>, Map<Integer, Integer>> valueChangesPerFluxValue =
  ...
  private <T> Map<Integer, Integer> computeValuesAtHeights(FluxValue<T> fluxValue) {
    return valueChangesPerFluxValue.computeIfAbsent(
        fluxValue,
        fv -> {
          ...
          for (int i = 0; i < valueChanges.length; i++) {
            valueChangeIndexAtHeight.put(
                getHistoricalMomentHeight(valueChanges[i].getHistoricalMoment()), i);
          }
