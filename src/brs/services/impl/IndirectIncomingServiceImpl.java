...
public class IndirectIncomingServiceImpl implements IndirectIncomingService {
  ...
  public IndirectIncomingServiceImpl(
      IndirectIncomingStore indirectIncomingStore, PropertyService propertyService) {
  ...
  public void processTransaction(Transaction transaction) {
    if (disabled) return;
    indirectIncomingStore.addIndirectIncomings(
        getIndirectIncomings(transaction).stream()
            .map(
                account ->
                    new IndirectIncomingStore.IndirectIncoming(
                        account, transaction.getId(), transaction.getHeight()))
            .collect(Collectors.toList()));
  }

  @Override
  public boolean isIndirectlyReceiving(Transaction transaction, long accountId) {
    // It would be confusing to have inconsistent behaviour so even when not loading from database
    // we should disable when told to do so.
