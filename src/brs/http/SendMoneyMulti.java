...
final class SendMoneyMulti extends CreateTransaction {
  ...
  JsonElement processRequest(HttpServletRequest req) throws BurstException {
    ...
    if (transactionArray.length > Constants.MAX_MULTI_OUT_RECIPIENTS
        || transactionArray.length < 2) {
      JsonObject response = new JsonObject();
    ...
    Attachment.PaymentMultiOutCreation attachment =
        new Attachment.PaymentMultiOutCreation(recipients, blockchain.getHeight());
