...
public final class JSONData {
  ...
  static JsonObject accountBalance(Account account) {
    ...
    if (account == null) {
      ...
      json.addProperty(UNCONFIRMED_BALANCE_NQT_RESPONSE,
          String.valueOf(account.getUnconfirmedBalanceNQT()));
  ...
  static JsonObject accountAsset(Account.AccountAsset accountAsset) {
    ...
    json.addProperty(UNCONFIRMED_QUANTITY_QNT_RESPONSE,
  ...
  static JsonObject block(Block block, boolean includeTransactions, int currentBlockchainHeight,
    ...
    json.addProperty(GENERATOR_PUBLIC_KEY_RESPONSE,
        Convert.toHexString(block.getGeneratorPublicKey()));
    ...
    json.addProperty(BLOCK_REWARD_RESPONSE,
        Convert.toUnsignedLong(blockReward / Constants.ONE_BURST));
    ...
    json.addProperty(AVERAGE_COMMITMENT_NQT_RESPONSE,
        Convert.toUnsignedLong(block.getAverageCommitment()));
    ...
    json.addProperty(GENERATION_SIGNATURE_RESPONSE,
        Convert.toHexString(block.getGenerationSignature()));

    if (block.getVersion() > 1) {
      json.addProperty(PREVIOUS_BLOCK_HASH_RESPONSE,
          Convert.toHexString(block.getPreviousBlockHash()));
    }
  ...
  static JsonObject unconfirmedTransaction(Transaction transaction) {
    ...
    json.addProperty(SENDER_PUBLIC_KEY_RESPONSE,
        Convert.toHexString(transaction.getSenderPublicKey()));
    ...
    if (transaction.getReferencedTransactionFullHash() != null) {
      json.addProperty(REFERENCED_TRANSACTION_FULL_HASH_RESPONSE,
    ...
    if (signature != null) {
      ...
      json.addProperty(SIGNATURE_HASH_RESPONSE,
          Convert.toHexString(Crypto.sha256().digest(signature)));
  ...
  static JsonObject at(AT at, boolean includeDetails) {
    ...
    if (includeDetails) {
      // These are immutable details, which we might want to avoid getting on every
      // call
