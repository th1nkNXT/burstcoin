...
public class APITransactionManagerImpl implements APITransactionManager {
  ...
  public APITransactionManagerImpl(
  ...
  @Override
  public JsonElement createTransaction(
    ...
    String referencedTransactionFullHash =
        Convert.emptyToNull(req.getParameter(REFERENCED_TRANSACTION_FULL_HASH_PARAMETER));
    String referencedTransactionId =
        Convert.emptyToNull(req.getParameter(REFERENCED_TRANSACTION_PARAMETER));
    ...
    String recipientPublicKeyValue =
        Convert.emptyToNull(ParameterParser.getRecipientPublicKey(req));
    ...
    if (attachment.getTransactionType().hasRecipient()) {
      EncryptedData encryptedData =
          parameterService.getEncryptedMessage(
              req,
              accountService.getAccount(recipientId),
              Convert.parseHexString(recipientPublicKeyValue));
      if (encryptedData != null) {
        encryptedMessage =
            new EncryptedMessage(
                encryptedData,
                !Parameters.isFalse(req.getParameter(MESSAGE_TO_ENCRYPT_IS_TEXT_PARAMETER)),
                blockchainHeight);
      }
    }
    ...
    if (messageValue != null) {
      ...
      try {
        message =
            messageIsText
                ? new Message(messageValue, blockchainHeight)
    ...
    if (recipientPublicKeyValue != null
      ...
      publicKeyAnnouncement =
          new PublicKeyAnnouncement(
              Convert.parseHexString(recipientPublicKeyValue), blockchainHeight);
    }
    ...
    try {
      ...
      if (secretPhrase != null) {
        ...
        transactionService.validate(
            transaction); // 2nd validate may be needed if validation requires id to be known
        ...
        response.addProperty(
            TRANSACTION_BYTES_RESPONSE, Convert.toHexString(transaction.getBytes()));
        response.addProperty(
      ...
      response.addProperty(
          UNSIGNED_TRANSACTION_BYTES_RESPONSE, Convert.toHexString(transaction.getUnsignedBytes()));
