...
final class SendMoneyMultiSame extends CreateTransaction {
  ...
  JsonElement processRequest(HttpServletRequest req) throws BurstException {
    ...
    if (recipientsArray.length > Constants.MAX_MULTI_SAME_OUT_RECIPIENTS
        || recipientsArray.length < 2) {
      JsonObject response = new JsonObject();
    ...
    Attachment.PaymentMultiSameOutCreation attachment =
        new Attachment.PaymentMultiSameOutCreation(recipients, blockchain.getHeight());
