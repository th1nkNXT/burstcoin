...
final class SendMoneyEscrow extends CreateTransaction {
  ...
  SendMoneyEscrow(
    ...
    super(
        new APITag[] {APITag.TRANSACTIONS, APITag.CREATE_TRANSACTION},
  ...
  JsonElement processRequest(HttpServletRequest req) throws BurstException {
    ...
    if (signersArray.length < 1
        || signersArray.length > 10
        || signersArray.length < requiredSigners) {
      JsonObject response = new JsonObject();
      response.addProperty(ERROR_CODE_RESPONSE, 4);
    ...
    try {
      ...
      response.addProperty(
          ERROR_DESCRIPTION_RESPONSE, "Invalid " + ESCROW_DEADLINE_PARAMETER + " parameter");
      return response;
    }

    Escrow.DecisionType deadlineAction =
        Escrow.stringToDecision(req.getParameter(DEADLINE_ACTION_PARAMETER));
    if (deadlineAction == null || deadlineAction == Escrow.DecisionType.UNDECIDED) {
      JsonObject response = new JsonObject();
      response.addProperty(ERROR_CODE_RESPONSE, 4);
      response.addProperty(
          ERROR_DESCRIPTION_RESPONSE, "Invalid " + DEADLINE_ACTION_PARAMETER + " parameter");
      return response;
    }

    Attachment.AdvancedPaymentEscrowCreation attachment =
        new Attachment.AdvancedPaymentEscrowCreation(
            amountNQT,
            (int) deadline,
            deadlineAction,
            (int) requiredSigners,
