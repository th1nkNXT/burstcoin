...
public class UnconfirmedTransactionStoreTest {
  ...
  public void setUp() {
    ...
    when(mockPropertyService.getInt(
            eq(Props.P2P_MAX_PERCENTAGE_UNCONFIRMED_TRANSACTIONS_FULL_HASH_REFERENCE)))
        .thenReturn(5);
    when(mockPropertyService.getInt(
            eq(Props.P2P_MAX_UNCONFIRMED_TRANSACTIONS_RAW_SIZE_BYTES_TO_SEND)))
        .thenReturn(175000);
    ...
    FluxCapacitor mockFluxCapacitor =
        QuickMocker.fluxCapacitorEnabledFunctionalities(
            FluxValues.PRE_POC2, FluxValues.DIGITAL_GOODS_STORE);
    ...
    t =
        new UnconfirmedTransactionStoreImpl(
            timeService, mockPropertyService, accountStoreMock, transactionDbMock);
  }
    ...
    for (int i = 1; i <= 100; i++) {
      Transaction transaction =
          new Transaction.Builder(
                  ...
                  i,
                  FEE_QUANT * 100,
                  timeService.getEpochTime() + 50000,
                  (short) 500,
                  ORDINARY_PAYMENT)
              .id(i)
              .senderId(123L)
  ...
  @DisplayName(
      "When a transaction got added by a peer, he won't get it reflected at him when getting"
  ...
  @DisplayName(
      "When a transactions got handed by a peer and we mark his fingerprints, he won't get it back"
          + " a second time")
  @Test
  public void transactionsMarkedWithPeerFingerPrintsWontGetReturnedToPeer()
  ...
  @DisplayName(
      "When The amount of unconfirmed transactions exceeds max size, and adding another then the"
          + " cache size stays the same")
  @Test
  public void
      numberOfUnconfirmedTransactionsOfSameSlotExceedsMaxSizeAddAnotherThenCacheSizeStaysMaxSize()
          throws ValidationException {
  ...
  @DisplayName(
      "When the amount of unconfirmed transactions exceeds max size, and adding another of a higher"
          + " slot, the cache size stays the same, and a lower slot transaction gets removed")
  @Test
  public void
      numberOfUnconfirmedTransactionsOfSameSlotExceedsMaxSizeAddAnotherThenCacheSizeStaysMaxSizeAndLowerSlotTransactionGetsRemoved()
          throws ValidationException {
    ...
    assertEquals(8192, t.getAll().size());
    assertEquals(
        8192 - 1, t.getAll().stream().filter(t -> t.getFeeNQT() == FEE_QUANT * 100).count());
  ...
  @DisplayName(
      "The unconfirmed transaction gets denied in case the account does not have enough unconfirmed"
          + " balance")
  @Test(expected = NotCurrentlyValidException.class)
  public void unconfirmedTransactionGetsDeniedForNotEnoughUnconfirmedBalance()
      throws ValidationException {
    when(mockBlockChain.getHeight()).thenReturn(20);
  ...
  @Test
  public void addingNewUnconfirmedTransactionWithSameIDResultsInNothingChanging()
      throws ValidationException {
    when(mockBlockChain.getHeight()).thenReturn(20);
    ...
    Builder transactionBuilder =
        new Builder(
                ...
                TestConstants.TEST_PUBLIC_KEY_BYTES,
                1,
                Constants.MAX_BALANCE_NQT - 100000,
                timeService.getEpochTime() + 50000,
                (short) 500,
                ORDINARY_PAYMENT)
            .id(1)
  ...
  @DisplayName(
      "When the maximum number of transactions with full hash reference is reached, following ones"
          + " are ignored")
  @Test
    ...
    for (int i = 1; i <= 414; i++) {
      Transaction transaction =
          new Transaction.Builder(
              ...
              .id(i)
              .senderId(123L)
              .referencedTransactionFullHash("b33f")
              .build();
      transaction.sign(TestConstants.TEST_SECRET_PHRASE);
      t.put(transaction, null);
    }
  ...
  @DisplayName(
      "When the maximum number of transactions for a slot size is reached, following ones are"
          + " ignored")
  @Test
