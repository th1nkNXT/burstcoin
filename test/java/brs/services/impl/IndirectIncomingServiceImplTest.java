...
public class IndirectIncomingServiceImplTest {
  private final AtomicReference<Consumer<List<IndirectIncomingStore.IndirectIncoming>>>
      addIndirectIncomingsRunnable = new AtomicReference<>();
  ...
  public void setUpIndirectIncomingServiceImplTest() {
    ...
    doAnswer(
        ...
        .when(indirectIncomingStore)
        .addIndirectIncomings(any());
    indirectIncomingService =
        new IndirectIncomingServiceImpl(indirectIncomingStore, propertyService);
  }

  @Test
  public void testIndirectIncomingServiceImplTestMultiOutTransaction()
      throws BurstException.NotValidException {
    addIndirectIncomingsRunnable.set(
        indirectIncomings -> {
          assertEquals(4, indirectIncomings.size());
          assertEquals(
              new HashSet<>(indirectIncomings).size(),
              indirectIncomings.size()); // Assert that there are no duplicates
        });
  ...
  @Test
  public void testIndirectIncomingServiceImplTestMultiOutSameTransaction()
      throws BurstException.NotValidException {
    addIndirectIncomingsRunnable.set(
        indirectIncomings -> {
          assertEquals(4, indirectIncomings.size());
    ...
    Attachment.PaymentMultiSameOutCreation attachment =
        mock(Attachment.PaymentMultiSameOutCreation.class);
    when(attachment.getRecipients()).thenReturn(recipients);
  ...
  public void testIndirectIncomingServiceImplTestOrdinaryTransaction() {
    addIndirectIncomingsRunnable.set(
        indirectIncomings -> assertEquals(0, indirectIncomings.size()));
