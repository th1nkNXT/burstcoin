...
public class GetConstantsHandlerTest extends AbstractGrpcTest {
  ...
  public void testGetConstantsHandler() {
    ...
    assertEquals(
        TransactionType.getTransactionTypes().size(), constants.getTransactionTypesList().size());
    constants
        .getTransactionTypesList()
        .forEach(
            transactionType -> {
              Map<Byte, TransactionType> subtypes =
                  TransactionType.getTransactionTypes().get((byte) transactionType.getType());
              assertNotNull(
                  "Transaction type " + transactionType.getType() + " does not exist", subtypes);
              assertEquals(
                  TransactionType.getTypeDescription((byte) transactionType.getType()),
                  ...
                  .forEach(
                      subtype -> {
                        TransactionType transactionSubtype =
                            subtypes.get((byte) subtype.getSubtype());
                        assertNotNull(
                            "Transaction subtype "
                                ...
                                + " does not exist",
                            transactionSubtype);
                        // Don't assume that its position in the map and its actual type are the
                        // same.
